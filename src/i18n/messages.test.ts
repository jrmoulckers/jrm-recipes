...
describe("matchRoutePattern", () => {
  ...
  it("fills dynamic segments", () => {
    ...
    expect(matchRoutePattern("/recipes/ada/apple-pie/edit")).toBe(
      "/recipes/:/:/edit",
    );
    expect(matchRoutePattern("/groups/sunday-club/settings")).toBe(
      "/groups/:/settings",
    );
  });
...
describe("namespacesForPathname", () => {
  ...
  it("adds the route's own namespaces", () => {
    ...
    expect(namespacesForPathname("/recipes/ada/pie/edit")).toContain(
      "recipeEditor",
    );
  });
...
describe("manifest integrity", () => {
  ...
  const referenced = [
    ...SHELL_NAMESPACES,
    ...Object.values(ROUTE_NAMESPACES).flat(),
  ];
  ...
  it("ships materially less than the whole catalog on a typical route", () => {
    ...
    const scoped = JSON.stringify(
      pickMessages(en, namespacesForPathname("/shopping")),
    ).length;
