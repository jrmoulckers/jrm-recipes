      ...
      describe(where, () => {
        for (const { fg, bg, label } of TEXT_PAIRS) {
          it(`${label} (--${fg} on --${bg}) >= ${TEXT_MIN}:1`, () => {
            const ratio = contrastRatio(token(tokens, fg, where), token(tokens, bg, where));
            expect(ratio, `${where} ${label} is ${ratio.toFixed(2)}:1`).toBeGreaterThanOrEqual(
              TEXT_MIN,
            );
          });
        }
        ...
        it(`form control border (--input on --background) >= ${UI_MIN}:1`, () => {
          ...
          expect(ratio, `${where} --input is ${ratio.toFixed(2)}:1`).toBeGreaterThanOrEqual(UI_MIN);
        });
      });
    }
  }
