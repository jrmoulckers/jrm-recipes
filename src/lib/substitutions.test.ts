...
describe('DIETARY_TAGS (i404 single source of truth)', () => {
  it('has a human label for every tag and no orphan labels', () => {
    expect(Object.keys(DIETARY_TAG_LABELS).sort()).toEqual([...DIETARY_TAGS].sort());
  });
...
describe('getSubstitutions', () => {
  ...
  it('filters substitutions by required dietary tags', () => {
    ...
    expect(veganButterSwaps.every((sub) => sub.dietaryTags?.includes('vegan'))).toBe(true);
  });
  ...
  it('filters to egg-free swaps for an egg-allergy cook', () => {
    ...
    expect(eggFreeMayoSwaps.every((sub) => sub.dietaryTags?.includes('egg-free'))).toBe(true);
    // The egg-free "Vegan mayo" swap survives. The egg-based options do not.
    expect(eggFreeMayoSwaps.map((sub) => sub.substitute)).toContain('Vegan mayo');
    expect(eggFreeMayoSwaps.map((sub) => sub.substitute)).not.toContain('Plain Greek yogurt');
  });
  ...
  it('filters to vegetarian swaps', () => {
    ...
    expect(vegetarianMayoSwaps.every((sub) => sub.dietaryTags?.includes('vegetarian'))).toBe(true);
  });
});
...
describe('filterSubstitutionsByDiet', () => {
  ...
  it('keeps only substitutions with all required tags', () => {
    expect(filterSubstitutionsByDiet(sampleSubs, ['vegan', 'dairy-free'])).toEqual([sampleSubs[1]]);
  });
});
...
describe('orderSubstitutionsByDiet', () => {
  it('stably moves substitutions matching all preferred tags first', () => {
    ...
    expect(
      orderSubstitutionsByDiet(sampleSubs, ['vegan', 'dairy-free']).map((sub) => sub.substitute),
    ).toEqual(['First match', 'Second match', 'First non-match', 'Second non-match']);
  });
