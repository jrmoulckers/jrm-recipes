...
describe('expandQueryTerms', () => {
  it('always includes the original normalized term first', () => {
    ...
    expect(expandQueryTerms('nonexistent-ingredient')).toEqual(['nonexistent-ingredient']);
  });
  ...
  it('caps the number of appended synonyms', () => {
    ...
    expect(expandQueryTerms('scallion').length).toBeLessThanOrEqual(1 + MAX_SYNONYM_EXPANSION);
  });
