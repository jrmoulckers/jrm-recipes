...
describe('buildContentSecurityPolicy', () => {
  ...
  describe("dev-only 'unsafe-eval'", () => {
    ...
    it('allows eval under `next dev` so HMR can hydrate', () => {
      ...
      expect(scriptSrc(buildContentSecurityPolicy('n'))).toContain("'unsafe-eval'");
    });

    it("never ships 'unsafe-eval' in the production policy", () => {
      vi.stubEnv('NODE_ENV', 'production');
      expect(scriptSrc(buildContentSecurityPolicy('n'))).not.toContain("'unsafe-eval'");
    });
  });
});
...
describe('clerkFrontendApiHost', () => {
  ...
  it('derives the FAPI host from a pk_test key', () => {
    expect(clerkFrontendApiHost(PK_TEST)).toBe('upbeat-cat-42.clerk.accounts.dev');
  });
