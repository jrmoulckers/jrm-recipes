...
describe('buildAliasIndex', () => {
  it('normalizes alias keys defensively', () => {
    const index = buildAliasIndex([{ alias: 'Yellow Onion', foodId: 'food_yellow', useCount: 3 }]);
  ...
  it('skips rows whose alias normalizes to empty', () => {
    const index = buildAliasIndex([{ alias: '   ', foodId: 'food_x', useCount: 1 }]);
...
describe('pickFoodId', () => {
  it('resolves an exact alias match (incl. Quantity-bearing mined phrasings)', () => {
    ...
    expect(pickFoodId('2 tbsp kosher salt, for finishing', index)).toBe('food_salt');
  });
  ...
  it('routes a variety alias to its own node, ahead of the curated fallback', () => {
    ...
    const index = buildAliasIndex([{ alias: 'yellow onion', foodId: varietyId, useCount: 2 }]);
  ...
  it('falls back to the curated static dataset (whole-word containment)', () => {
    ...
    expect(pickFoodId('2 cloves garlic, minced', empty)).toBe(canonicalFood('garlic')?.id);
    expect(pickFoodId('2 cloves garlic, minced', empty)).toBe(foodNodeId('Garlic'));
  });
