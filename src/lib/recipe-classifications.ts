...
export function groupRecipeClassifications(
  ...
  const legacyCanonical = legacyCuisine?.trim()
    ? canonicalizeTag(legacyCuisine, "cuisine")
    : null;
  ...
  if (
    grouped.cuisine.length === 0 &&
    legacyCanonical &&
    !seen.has(legacyCanonical.slug)
  ) {
  ...
  for (const [category, values] of Object.entries(grouped)) {
    ...
    if (category === "cuisine" && legacyCanonical) {
      const legacyIndex = values.findIndex(
        (value) => value.slug === legacyCanonical.slug,
      );
      if (legacyIndex > 0) {
