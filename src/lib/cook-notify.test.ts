...
describe('cookTimerNotificationUrl', () => {
  it("hangs Cook Mode off the recipe's canonical namespaced path", () => {
    expect(cookTimerNotificationUrl('/recipes/ada/apple-pie')).toBe('/recipes/ada/apple-pie/cook');
  });
});
...
describe('requestTimerNotificationPermission', () => {
  it("returns 'denied' when notifications are unsupported", async () => {
    await expect(requestTimerNotificationPermission(undefined)).resolves.toBe('denied');
  });
