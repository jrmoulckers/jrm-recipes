...
describe('recipe presets', () => {
  ...
  describe('isPresetActive', () => {
    it('is false when no params match', () => {
      expect(isPresetActive(new URLSearchParams(''), preset('weeknight'))).toBe(false);
    });
    ...
    it('requires every owned param to be present', () => {
      ...
      const full = new URLSearchParams('maxTime=30&difficulty=easy&sort=quickest');
  ...
  describe('togglePreset', () => {
    ...
    it('clears all owned params when already active', () => {
      const active = new URLSearchParams('maxTime=30&difficulty=easy&sort=quickest&q=pasta');
