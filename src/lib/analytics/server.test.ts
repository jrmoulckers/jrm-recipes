...
import { captureServer, identifyServer, aliasServer, getAllFlags, getFlag } from './server';
...
describe('feature flags', () => {
  ...
  it('getFlag returns the flag value or the provided fallback', async () => {
    ...
    await expect(getFlag('user_1', 'empty_state_cta', 'control')).resolves.toBe('sample_shortcut');
    await expect(getFlag('user_1', 'missing_flag', 'control')).resolves.toBe('control');
  });
});
