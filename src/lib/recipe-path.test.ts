...
describe("recipeDetailPath", () => {
  it("builds the canonical namespaced path when the cook slug is known", () => {
    expect(
      recipeDetailPath({ id: "rec_123", slug: "apple-pie", cook: "ada" }),
    ).toBe("/recipes/ada/apple-pie");
  });
  ...
  it("falls back to the id when a recipe has no slug", () => {
    expect(recipeDetailPath({ id: "rec_123", slug: null, cook: "ada" })).toBe(
      "/recipes/rec_123",
    );
  });
});
...
describe("recipeRevalidationPaths", () => {
  ...
  it("emits a single path when there is nothing to fan out to", () => {
    expect(
      recipeRevalidationPaths({ id: "rec_123", slug: "apple-pie" }),
    ).toEqual(["/recipes/apple-pie"]);
  });
});
