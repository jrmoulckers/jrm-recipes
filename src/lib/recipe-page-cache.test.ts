...
describe('isRecipePageRequest', () => {
  it('matches a recipe detail document navigation', () => {
    expect(isRecipePageRequest(makeRequest(`${ORIGIN}/recipes/apple-pie`))).toBe(true);
  });

  it('matches the Cook Mode document navigation', () => {
    expect(isRecipePageRequest(makeRequest(`${ORIGIN}/recipes/apple-pie/cook`))).toBe(true);
  });
  ...
  it('matches an RSC payload request via the RSC header', () => {
    ...
    expect(isRecipePageRequest(makeRequest(`${ORIGIN}/recipes/apple-pie`, '', true))).toBe(true);
  });

  it('matches an RSC payload request via the ?_rsc= param', () => {
    expect(
      isRecipePageRequest(makeRequest(`${ORIGIN}/recipes/apple-pie/cook?_rsc=abc123`, '')),
    ).toBe(true);
  });

  it('ignores a recipe subresource that is neither a document nor RSC', () => {
    expect(isRecipePageRequest(makeRequest(`${ORIGIN}/recipes/apple-pie`, 'script'))).toBe(false);
  });

  it('does not cache the edit page', () => {
    expect(isRecipePageRequest(makeRequest(`${ORIGIN}/recipes/apple-pie/edit`))).toBe(false);
  });
  ...
  it('does not match deeper paths under a recipe id', () => {
    expect(isRecipePageRequest(makeRequest(`${ORIGIN}/recipes/apple-pie/cook/steps`))).toBe(false);
  });
