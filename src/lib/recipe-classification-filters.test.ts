...
describe("classificationParam / classificationValue", () => {
  ...
  it("filters cuisines by display name and everything else by slug", () => {
    expect(
      classificationValue(option("italian", "cuisine", 1, "Italian")),
    ).toBe("Italian");
    expect(classificationValue(option("dinner", "meal", 1, "Dinner"))).toBe(
      "dinner",
    );
  });
});
...
describe("isClassificationActive", () => {
  it("matches case-insensitively within the classification's own param", () => {
    ...
    expect(isClassificationActive(params, option("quick", "general"))).toBe(
      true,
    );
  ...
  it("does not match a value that lives under a different param", () => {
    ...
    expect(isClassificationActive(params, option("dinner", "meal"))).toBe(
      false,
    );
  });
});
...
describe("pickBrowseClassifications", () => {
  ...
  it("orders meals first, then cuisine, dietary, and general tags", () => {
    expect(
      pickBrowseClassifications(items, new URLSearchParams(), 10).map(
        (item) => item.slug,
      ),
  ...
  it("ranks by usage inside a category and breaks ties by name", () => {
    ...
    expect(picked.map((item) => item.slug)).toEqual([
      "supper",
      "brunch",
      "dinner",
    ]);
  ...
  it("never returns more than the limit", () => {
    expect(
      pickBrowseClassifications(items, new URLSearchParams(), 3),
    ).toHaveLength(3);
  });
});
