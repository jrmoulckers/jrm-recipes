...
describe('detectStepHazards (issue #423)', () => {
  ...
  it('detects heat from step-text keywords', () => {
    expect(detectStepHazards({ text: 'Place the tray in the oven for 20 minutes.' })).toEqual([
      'heat',
    ]);
    expect(detectStepHazards({ text: 'Bring the water to a boil, then add pasta.' })).toEqual([
      'heat',
    ]);
  });

  it('detects sharp from step-text keywords', () => {
    expect(detectStepHazards({ text: 'Slice the carrots into thin rounds.' })).toEqual(['sharp']);
    expect(detectStepHazards({ text: 'Grate the cheese over the top.' })).toEqual(['sharp']);
  });

  it('returns both hazards in a stable order (heat then sharp)', () => {
    expect(detectStepHazards({ text: 'Sear the steak, then slice it thinly.' })).toEqual([
      'heat',
      'sharp',
    ]);
  });
