...
describe('formatIngredientLine', () => {
  it('renders quantity, unit and item', () => {
    expect(formatIngredientLine(ing('flour', { quantity: 2, unit: 'cup' }))).toBe('2 cup flour');
  });

  it('renders a quantity range', () => {
    expect(formatIngredientLine(ing('water', { quantity: 1, quantityMax: 2, unit: 'cup' }))).toBe(
      '1–2 cup water',
    );
  });
