...
export function ingredientRouteIdentity(
  ...
  const normalizedItem =
    normalizeIngredient(ingredient.item) || normalizeItemName(ingredient.item);
  return {
...
export function findIngredientRoute(
  ...
  return (
    routes.find((route) => route.normalizedItem === identity.normalizedItem) ??
    null
  );
}
...
export function resolveIngredientDestination(
  ...
  const preferred =
    route && activeListIds.has(route.preferredListId)
      ? route.preferredListId
      : null;
  const promoted =
    route?.alternativeListIds.find((id) => activeListIds.has(id)) ?? null;
  const listId = preferred ?? promoted ?? defaultListId;
  const alternativeListIds =
    route?.alternativeListIds.filter(
      (id, index, ids) =>
        id !== listId && activeListIds.has(id) && ids.indexOf(id) === index,
    ) ?? [];
  return {
    listId,
...
>(
  ...
  for (const item of items) {
    const { listId } = resolveIngredientDestination(
      item,
      routes,
      activeListIds,
      defaultListId,
