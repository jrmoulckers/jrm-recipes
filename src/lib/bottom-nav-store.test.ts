...
describe('bottom-nav store', () => {
  ...
  it('reorders with moveUp / moveDown', () => {
    ...
    expect(useBottomNavStore.getState().pinned).toEqual(['home', 'plan', 'recipes', 'shopping']);

    useBottomNavStore.getState().moveDown('home');
    expect(useBottomNavStore.getState().pinned).toEqual(['plan', 'home', 'recipes', 'shopping']);
  });
