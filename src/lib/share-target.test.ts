...
describe('pickSharedUrl', () => {
  it('prefers a direct http(s) url', () => {
    expect(pickSharedUrl('https://example.com/recipe')).toBe('https://example.com/recipe');
    expect(pickSharedUrl('http://example.com/recipe')).toBe('http://example.com/recipe');
  });
  ...
  it('digs a bare url out of shared free text', () => {
    expect(pickSharedUrl('Check this out https://example.com/marinara please')).toBe(
      'https://example.com/marinara',
    );
  });

  it('falls through url -> text -> title', () => {
    expect(pickSharedUrl(null, 'no link here', 'Grandma https://example.com/x')).toBe(
      'https://example.com/x',
    );
  });
...
describe('isShareableImage', () => {
  ...
  it('rejects non-image, empty, and oversize files', () => {
    ...
    expect(isShareableImage({ type: 'image/png', size: SHARED_IMAGE_MAX_BYTES + 1 })).toBe(false);
...
describe('safeSharedImageUrl', () => {
  it('accepts only https Cloudinary URLs', () => {
    expect(safeSharedImageUrl('https://res.cloudinary.com/demo/image/upload/x.jpg')).toBe(
  ...
  it('rejects other hosts, schemes, and junk', () => {
    ...
    expect(safeSharedImageUrl('http://res.cloudinary.com/demo/x.jpg')).toBeUndefined();
