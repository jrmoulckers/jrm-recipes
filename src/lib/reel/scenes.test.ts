...
describe('formatIngredientLine', () => {
  it('combines quantity, unit and item', () => {
    expect(formatIngredientLine({ quantity: 2, unit: 'cup', item: 'flour' })).toBe('2 cup flour');
  });

  it('drops trailing zeros from fractional quantities', () => {
    expect(formatIngredientLine({ quantity: 0.5, unit: 'tsp', item: 'salt' })).toBe('0.5 tsp salt');
  });
...
describe('metaChips', () => {
  it('builds chips for time, servings, difficulty and cuisine', () => {
    ...
    expect(chips.map((c) => c.label)).toEqual(['25 min', 'Serves 4', 'Easy', 'American']);
...
describe('selectKeyIngredients', () => {
  it('caps to the limit and puts core ingredients before optional ones', () => {
    ...
    expect(picked.every((p, i) => i === 0 || !picked[i - 1]!.optional || p.optional)).toBe(true);
  });
...
describe('buildReelScenes', () => {
  ...
  it('puts the site url on the outro', () => {
    ...
    expect(REEL_SITE_URL).toBe(process.env.NEXT_PUBLIC_SITE_URL ?? 'heirloom.jrmoulckers.com');
  });
});
...
describe('reelImageUrl', () => {
  it('injects a 9:16 fill transform for Cloudinary urls', () => {
    const out = reelImageUrl('https://res.cloudinary.com/demo/image/upload/v123/pancakes.jpg');
    expect(out).toContain('/upload/f_auto,q_auto,w_1080,h_1920,c_fill,g_auto/');
    expect(out).toContain('pancakes.jpg');
  });

  it('respects custom dimensions', () => {
    const out = reelImageUrl('https://res.cloudinary.com/demo/image/upload/v1/a.jpg', 540, 960);
    expect(out).toContain('w_540,h_960');
  });

  it('leaves non-Cloudinary urls untouched and passes through null', () => {
    expect(reelImageUrl('https://example.com/a.jpg')).toBe('https://example.com/a.jpg');
