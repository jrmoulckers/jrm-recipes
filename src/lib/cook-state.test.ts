...
describe('timerStatusText', () => {
  ...
  const t = (key: string, values?: { time: string }) => (values ? `${key}:${values.time}` : key);
  ...
  it('routes each timer status to its message key with a formatted countdown', () => {
    ...
    expect(timerStatusText(timer({ status: 'running', remaining: 65 }), t)).toBe('remaining:1:05');
    expect(timerStatusText(timer({ status: 'paused', remaining: 65 }), t)).toBe('paused:1:05');
    expect(timerStatusText(timer({ status: 'idle', duration: 120 }), t)).toBe('ready:2:00');
  });
});
