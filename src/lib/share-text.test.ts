...
describe('shareText', () => {
  ...
  it('falls back gracefully when the author is missing', () => {
    expect(shareText({ title: 'Apple Pie' })).toBe('Apple Pie. A family recipe on Heirloom.');
  ...
  it('appends the url for the clipboard fallback', () => {
    expect(
      shareMessageWithUrl({ title: 'Apple Pie' }, 'https://heirloom.app/recipes/apple-pie'),
    ).toBe('Apple Pie. A family recipe on Heirloom. https://heirloom.app/recipes/apple-pie');
  });
