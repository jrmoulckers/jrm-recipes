...
describe('ingredientRouteIdentity', () => {
  it('prefers a canonical food id', () => {
    expect(ingredientRouteIdentity({ item: 'Yellow onion', foodId: 'food-onion' })).toEqual({
      key: 'food:food-onion',
      foodId: 'food-onion',
...
describe('findIngredientRoute', () => {
  it('matches canonical identity before normalized text', () => {
    expect(findIngredientRoute({ item: 'Yellow onion', foodId: 'food-onion' }, routes)?.id).toBe(
      'route-onion',
    );
  });

  it('reuses a normalized legacy rule when no canonical rule exists', () => {
    expect(findIngredientRoute({ item: 'Onion' }, routes)?.id).toBe('route-onion');
  });
});
