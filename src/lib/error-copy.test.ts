...
describe('friendlyError', () => {
  ...
  it("unwraps an Error instance's message", () => {
    expect(friendlyError(new Error('NOT_AUTHENTICATED'))).toBe('Please sign in to do that.');
  ...
  it('honours a custom fallback and never returns an empty string', () => {
    expect(friendlyError('', "Couldn't save your note")).toBe("Couldn't save your note");
    expect(friendlyError('', '   ')).toBe(DEFAULT_ERROR_COPY);
  });
});
