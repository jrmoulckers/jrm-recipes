...
describe("recipeFallbackImage", () => {
  ...
  it.each([
    ...
    "prioritizes the specific course in %j over broad descriptors",
    (tags, family) => {
  ...
  it.each([
    ...
    "prioritizes the explicit course in %s over a dish keyword",
    (title, family) => {
  ...
  it("keeps ambiguous curry titles in the generic rotation", () => {
    expect(
      recipeFallbackImage("chicken-curry", { title: "Chicken Curry" }),
    ).toMatch(
  ...
  it("keeps unmatched recipes inside the generic rotation", () => {
    const selected = new Set(
      Array.from({ length: 40 }, (_, index) =>
        recipeFallbackImage(`unknown-${index}`),
      ),
    );
