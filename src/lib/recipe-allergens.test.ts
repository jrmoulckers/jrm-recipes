...
import { ingredientAllergens, unionIngredientAllergens } from './recipe-allergens';
...
describe('ingredientAllergens', () => {
  it('uses structured food-graph tokens when present (source of truth)', () => {
    ...
    expect(ingredientAllergens({ item: 'butter', foodAllergens: ['soy'] })).toEqual(['soy']);
  });
  ...
  it("treats an empty structured array as 'resolved, carries none' (suppresses text)", () => {
    ...
    expect(ingredientAllergens({ item: 'peanut butter', foodAllergens: [] })).toEqual([]);
  });

  it('falls back to text detection when structured tokens are null', () => {
    expect(ingredientAllergens({ item: '2 cups whole milk', foodAllergens: null })).toEqual([
      'dairy',
    ]);
  });
