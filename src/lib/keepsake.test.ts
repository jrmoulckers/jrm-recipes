...
import { KEEPSAKE_NOTE_MAX, buildKeepsakePath, parseKeepsakeMessage } from './keepsake';
...
describe('buildKeepsakePath', () => {
  ...
  it('builds a bare keepsake path when there is no message', () => {
    expect(buildKeepsakePath(RECIPE_PATH, {})).toBe('/recipes/nonna/banana-bread/keepsake');
  });
...
describe('parseKeepsakeMessage', () => {
  ...
  it('caps the note length', () => {
    ...
    expect(parseKeepsakeMessage({ note: long }).note).toHaveLength(KEEPSAKE_NOTE_MAX);
  });

  it('takes the first value when a param repeats', () => {
    expect(parseKeepsakeMessage({ from: ['Nonna', 'someone'] }).from).toBe('Nonna');
  });

  it('preserves simple line breaks but collapses excess blank lines', () => {
    expect(parseKeepsakeMessage({ note: 'Line 1\n\n\n\nLine 2' }).note).toBe('Line 1\n\nLine 2');
  });
});
