...
describe('isRecipeImageRequest', () => {
  it('matches direct cross-origin Cloudinary image requests', () => {
    expect(
      isRecipeImageRequest(makeRequest('https://res.cloudinary.com/demo/image/upload/v1/hero.jpg')),
    ).toBe(true);
  });
  ...
  it('matches Next.js optimizer requests proxying a Cloudinary source', () => {
    ...
    expect(
      isRecipeImageRequest(
        makeRequest(optimizerUrl('https://res.cloudinary.com/demo/image/upload/v1/step-3.jpg')),
      ),
    ).toBe(true);
  });
  ...
  it('ignores Cloudinary videos (only images belong in the bounded cache)', () => {
    expect(
      isRecipeImageRequest(
        makeRequest('https://res.cloudinary.com/demo/video/upload/v1/clip.mp4', 'video'),
      ),
    ).toBe(false);
  });
  ...
  it('leaves non-Cloudinary optimized images on the default next-image route', () => {
    ...
    expect(
      isRecipeImageRequest(makeRequest(optimizerUrl('https://img.clerk.com/avatar.png'))),
    ).toBe(false);
  });

  it('ignores optimizer requests for local (same-origin) images', () => {
    expect(isRecipeImageRequest(makeRequest(optimizerUrl('/icons/logo.png')))).toBe(false);
  });

  it('ignores an optimizer request with no url param', () => {
    expect(isRecipeImageRequest(makeRequest('https://heirloom.test/_next/image?w=640&q=75'))).toBe(
      false,
    );
  });
  ...
  it('does not match hostnames that merely start with the Cloudinary host', () => {
    expect(
      isRecipeImageRequest(makeRequest('https://res.cloudinary.com.evil.example/steal.jpg')),
    ).toBe(false);
  });

  it('ignores same-origin navigations and other subresources', () => {
    expect(
      isRecipeImageRequest(makeRequest('https://heirloom.test/recipes/apple-pie', 'document')),
    ).toBe(false);
    expect(
      isRecipeImageRequest(makeRequest('https://heirloom.test/_next/static/chunk.js', 'script')),
    ).toBe(false);
  });
  ...
  it('accepts a real Request (structural compatibility)', () => {
    ...
    const req = new Request('https://res.cloudinary.com/demo/image/upload/v1/hero.jpg');
