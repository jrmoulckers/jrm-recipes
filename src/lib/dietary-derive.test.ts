...
describe('deriveDietaryTags', () => {
  ...
  it('does NOT tag a recipe with flour as gluten-free', () => {
    expect(deriveDietaryTags(['all-purpose flour', 'water'])).not.toContain('gluten-free');
  });

  it('does NOT tag a recipe with eggs as egg-free', () => {
    expect(deriveDietaryTags(['3 large eggs', 'milk'])).not.toContain('egg-free');
  });
  ...
  it('only ever derives the three detectable -free tags', () => {
    expect(DERIVED_DIETARY_TAGS).toEqual(['dairy-free', 'gluten-free', 'egg-free']);
