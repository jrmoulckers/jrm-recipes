...
import { hasReacted, reactionGlyph, toggleReactionState, type ReactionCount } from './reactions';
...
describe('toggleReactionState (issue #342)', () => {
  ...
  it('increments an existing emoji the viewer had not reacted to', () => {
    const counts: ReactionCount[] = [{ emoji: 'love', count: 2, reacted: false }];
  ...
  it('decrements but keeps the emoji when others still reacted', () => {
    const counts: ReactionCount[] = [{ emoji: 'clap', count: 3, reacted: true }];
  ...
  it('keeps the fixed emoji order after a toggle', () => {
    const counts: ReactionCount[] = [{ emoji: 'party', count: 1, reacted: false }];
    const next = toggleReactionState(counts, 'love');
    expect(next.map((c) => c.emoji)).toEqual(['love', 'party']);
  });

  it('does not mutate the input array', () => {
    const counts: ReactionCount[] = [{ emoji: 'love', count: 1, reacted: false }];
