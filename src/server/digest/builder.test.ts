...
describe('buildWeeklyDigest', () => {
  ...
  it('counts a recipe updated (but not created) in the window as updated', () => {
    const digest = buildWeeklyDigest({
      groups,
      recipes: [recipe({ id: 'old', createdAt: daysAgo(40), updatedAt: daysAgo(2) })],
      now: NOW,
    });
  ...
  it('excludes activity older than the window', () => {
    const digest = buildWeeklyDigest({
      groups,
      recipes: [recipe({ id: 'stale', createdAt: daysAgo(40), updatedAt: daysAgo(40) })],
      now: NOW,
    });
  ...
  it('honors a custom window length', () => {
    const recipes = [recipe({ id: 'd3', createdAt: daysAgo(3), updatedAt: daysAgo(3) })];
    expect(buildWeeklyDigest({ groups, recipes, now: NOW, windowDays: 2 })).toBeNull();
    expect(buildWeeklyDigest({ groups, recipes, now: NOW, windowDays: 7 })?.totalNew).toBe(1);
  });
});
