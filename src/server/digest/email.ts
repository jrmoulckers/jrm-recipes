...
export function renderDigestEmail(digest: WeeklyDigest): RenderedEmail {
  ...
  const textLines: string[] = [`Here's what's new across your ${brand.name} family cookbooks:`, ''];
 ...
 */
export function createResendProvider(apiKey: string, from: string = DEFAULT_FROM): EmailProvider {
  return {
    name: 'resend',
    async send(message) {
