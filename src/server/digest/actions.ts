...
export type DigestPrefResult = { ok: true; optedIn: boolean } | { ok: false; error: string };
 ...
 */
export async function setWeeklyDigestOptInAction(optedIn: boolean): Promise<DigestPrefResult> {
  if (!isDbConfigured()) {
  ...
  try {
    await db.update(users).set({ weeklyDigestOptIn: optedIn }).where(eq(users.id, user.id));
