...
import type { CreateCookAlongInput, RsvpInput, UpdateCookAlongInput } from './validation';
 ...
 */
export async function createCookAlong(input: CreateCookAlongInput, user: User): Promise<CookAlong> {
...
export async function updateCookAlong(input: UpdateCookAlongInput, user: User): Promise<void> {
  await db.transaction(async (tx: Tx) => {
    ...
    const canEdit = event.hostId === user.id || role === 'owner' || role === 'admin';
...
export async function deleteCookAlong(cookAlongId: string, user: User): Promise<void> {
  await db.transaction(async (tx: Tx) => {
    const event = await tx.query.cookAlongs.findFirst({
    ...
    const role = await memberRoleOf(tx, event.groupId, user.id);
    const canDelete = event.hostId === user.id || role === 'owner' || role === 'admin';
  ...
  for (const event of due) {
    ...
    await db.transaction(async (tx: Tx) => {
      ...
      const [claimed] = await tx
        ...
        .where(and(eq(cookAlongs.id, event.id), isNull(cookAlongs.reminderSentAt)))
