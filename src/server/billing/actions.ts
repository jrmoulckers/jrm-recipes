...
import { redeemGiftCode, GIFT_NOT_FOUND, GIFT_ALREADY_REDEEMED } from './gifting';
...
const NO_DB = 'Billing needs a database. Set DATABASE_URL (see .env.example) to continue.';
const NO_CUSTOMER =
  "You don't have a billing account yet. Start a plan from the pricing page first.";
const GENERIC = "We couldn't reach Stripe just now. Please try again in a moment.";
 ...
 */
export async function createCheckoutSessionAction(planId: string): Promise<BillingActionResult> {
...
export type RedeemGiftResult =
  { ok: true; planId: PlanId; durationMonths: number } | { ok: false; error: string };
