...
describe('waitlistInput validation', () => {
  ...
  it('only accepts known source tags', () => {
    expect(waitlistInput.safeParse({ email: 'a@b.com', source: 'hero' }).success).toBe(true);
    expect(waitlistInput.safeParse({ email: 'a@b.com', source: 'evil' }).success).toBe(false);
  });
});
...
describe('addToWaitlist', () => {
  it("returns 'unavailable' and touches no DB when unconfigured", async () => {
    ...
    expect(await addToWaitlist({ email: 'a@b.com', source: 'landing' })).toBe('unavailable');
    expect(db.insert).not.toHaveBeenCalled();
  });

  it("returns 'created' when a new row is inserted", async () => {
    state.rows = [{ id: 'w1' }];
    expect(await addToWaitlist({ email: 'a@b.com', source: 'landing' })).toBe('created');
  });

  it("returns 'duplicate' when the email already exists (no row returned)", async () => {
    state.rows = [];
    expect(await addToWaitlist({ email: 'a@b.com', source: 'landing' })).toBe('duplicate');
  });
});
