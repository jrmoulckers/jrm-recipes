...
import { canonicalizeQuery, createSavedSearch, deleteSavedSearch } from './mutations';
...
describe('canonicalizeQuery (#278)', () => {
  it('normalizes params and drops the contextual default sort', () => {
    ...
    expect(canonicalizeQuery('tag=quick&tag=vegan&sort=newest')).toBe('tag=quick&tag=vegan');
  });
...
describe('createSavedSearch (#278)', () => {
  ...
  it('rejects a search with no active filters', async () => {
    await expect(createSavedSearch({ name: 'Empty', query: 'sort=newest' }, user)).rejects.toThrow(
      'EMPTY_SEARCH',
    );
  ...
  it('enforces the per-user cap for a new name', async () => {
    ...
    await expect(createSavedSearch({ name: 'Weeknight', query: 'q=pasta' }, user)).rejects.toThrow(
      'LIMIT_REACHED',
    );
  ...
  it('skips the cap check when overwriting an existing name', async () => {
    ...
    const result = await createSavedSearch({ name: 'Quick veg', query: 'q=soup' }, user);
...
describe('deleteSavedSearch (#278)', () => {
  ...
  it('throws NOT_FOUND when nothing was deleted', async () => {
    ...
    await expect(deleteSavedSearch('missing', user)).rejects.toThrow('NOT_FOUND');
  });
