...
describe('canListInGroupCookbook', () => {
  ...
  it('shows group and public recipes to members', () => {
    expect(canListInGroupCookbook(recipe({ visibility: 'group' }), otherMember, true)).toBe(true);
    expect(canListInGroupCookbook(recipe({ visibility: 'public' }), otherMember, true)).toBe(true);
  });

  it('only shows published public recipes to non-members', () => {
    // A public draft must not leak to the public cookbook view.
    expect(
      canListInGroupCookbook(recipe({ visibility: 'public', status: 'draft' }), null, false),
    ).toBe(false);
    expect(
      canListInGroupCookbook(recipe({ visibility: 'public', status: 'published' }), null, false),
    ).toBe(true);
    // Group-visibility recipes are never shown to non-members.
    expect(canListInGroupCookbook(recipe({ visibility: 'group' }), null, false)).toBe(false);
  });
});
