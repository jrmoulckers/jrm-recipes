...
import { addMemberInput, createInviteLinkInput, groupInput, updateRoleInput } from './validation';
...
describe('groupInput', () => {
  ...
  it('requires a name', () => {
    expect(() => groupInput.parse({ name: ' ' })).toThrow(/Give your group a name/);
  });
});
...
describe('addMemberInput', () => {
  ...
  it('rejects empty identifiers', () => {
    expect(() => addMemberInput.parse({ identifier: ' ' })).toThrow(/Enter a handle or email/);
  });
});
...
describe('createInviteLinkInput (issue #343)', () => {
  ...
  it('coerces and bounds expiry and max uses', () => {
    expect(createInviteLinkInput.parse({ expiresInDays: '30', maxUses: '10' })).toMatchObject({
      expiresInDays: 30,
      maxUses: 10,
    });
