...
import { groupInvitations, groupMembers, type MemberRole, type User } from '~/server/db/schema';
import { acceptInvitation, createInvitation, revokeInvitation } from './mutations';
...
describe('createInvitation (issue #181)', () => {
  ...
  it('rejects a non-member actor with FORBIDDEN', async () => {
    ...
    await expect(createInvitation('family', owner, { email: 'x@example.com' })).rejects.toThrow(
      'FORBIDDEN',
    );
  });
  ...
  it('refuses to invite someone who is already a member', async () => {
    const tx = fakeTx({
      memberships: [ownerMember(), { id: 'm_t', role: 'member', userId: 't1', groupId: 'group_1' }],
      targetUser: { id: 't1' },
    });
    runWith(tx);
    await expect(
  ...
  it('refuses a duplicate pending invite', async () => {
    ...
    runWith(tx);
    await expect(createInvitation('family', owner, { email: 'dup@example.com' })).rejects.toThrow(
      'ALREADY_INVITED',
    );
  });
});
...
describe('acceptInvitation (issue #181)', () => {
  ...
  it("won't reuse a consumed (accepted) invite for a non-member", async () => {
    ...
    runWith(tx);
    await expect(acceptInvitation('tok_1', joiner)).rejects.toThrow('ALREADY_ACCEPTED');
    expect(tx.insert).not.toHaveBeenCalled();
  });
});
...
describe('revokeInvitation (issue #181)', () => {
  ...
  it("won't revoke an accepted invite", async () => {
    ...
    runWith(tx);
    await expect(revokeInvitation('family', owner, 'inv_1')).rejects.toThrow('NOT_PENDING');
  });

  it('rejects a non-manager', async () => {
    runWith(fakeTx({ memberships: [] }));
    await expect(revokeInvitation('family', admin, 'inv_1')).rejects.toThrow('FORBIDDEN');
  });
});
...
describe('acceptInvitation seat enforcement (#325)', () => {
  ...
  it('rejects joining beyond the seat limit and seats nobody', async () => {
    ...
    await expect(acceptInvitation('tok_1', joiner)).rejects.toThrow('SEAT_LIMIT_REACHED');
    expect(tx.insert).not.toHaveBeenCalled();
  });
