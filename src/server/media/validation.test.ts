...
import { mediaPublicId, mediaUrl, recordUploadInput, updateAltTextInput } from './validation';

describe('mediaUrl', () => {
  it('accepts an allowlisted delivery host', () => {
    expect(mediaUrl.safeParse('https://res.cloudinary.com/demo/image/upload/a.jpg').success).toBe(
      true,
    );
  });

  it("rejects an off-allowlist host so a stored image can't beacon viewers", () => {
    expect(mediaUrl.safeParse('https://evil.test/track.jpg').success).toBe(false);
  });
...
describe('updateAltTextInput', () => {
  it('requires an id', () => {
    expect(updateAltTextInput.safeParse({ altText: 'A pie' }).success).toBe(false);
  });
