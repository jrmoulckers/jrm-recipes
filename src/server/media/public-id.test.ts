...
describe('cloudinaryRefFromUrl', () => {
  ...
  it('handles a URL with no version and no transforms', () => {
    expect(cloudinaryRefFromUrl('https://res.cloudinary.com/demo/image/upload/a.jpg')).toEqual({
      publicId: 'a',
      resourceType: 'image',
    });
  });
  ...
  it('refuses URLs on other hosts', () => {
    ...
    expect(
      cloudinaryRefFromUrl('https://res.cloudinary.com.evil.example/image/upload/v1/a.jpg'),
    ).toBeNull();
  });
  ...
  it('returns null rather than guessing on malformed input', () => {
    ...
    expect(cloudinaryRefFromUrl('https://res.cloudinary.com/demo/image/upload/')).toBeNull();
  });
});
