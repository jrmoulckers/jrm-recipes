...
describe('getAssetUsage', () => {
  it("does not count a foreign group's use of the caller's photo", async () => {
    ...
    const groupWhere = state.countWheres.find(({ table }) => table === groups)?.condition as
      SQL | undefined;
  ...
  it('returns NOT_FOUND for a foreign-owned asset without an existence oracle', async () => {
    ...
    await expect(getAssetUsage('foreign-asset', other)).rejects.toThrow('NOT_FOUND');
    ...
    expect(ownershipQuery.params).toEqual(expect.arrayContaining(['foreign-asset', other.id]));
  });
});
