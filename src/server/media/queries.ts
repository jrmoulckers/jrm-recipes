 ...
 * never in. So `groups` is restricted to the caller's own memberships, and
 * most other surfaces to rows the caller owns outright. Source attachments
 * also count on recipes the caller is an accepted co-creator of, because that
 * is a live use they can already see and edit. Undercounting a surface someone
 * else controls is the safe direction to be wrong in — the warning is
 * advisory, and the delete never cascades either way.
 *
