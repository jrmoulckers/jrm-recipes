...
import { getReactionsForTargets, toggleReaction, type ReactionTargetType } from './reactions';
...
describe('getReactionsForTargets block filtering (#355)', () => {
  ...
  it('counts and names every reactor when nothing is blocked', async () => {
    ...
    expect(target.counts).toEqual([{ emoji: 'love', count: 2, reacted: false }]);
  ...
  it('drops a blocked reactor from both the count and the reactor names', async () => {
    const map = await getReactionsForTargets('comment', ['c1'], 'viewer_1', new Set(['blocked_1']));
    const target = map.get('c1')!;
    expect(target.counts).toEqual([{ emoji: 'love', count: 1, reacted: false }]);
...
function notifications(tx: ReturnType<typeof fakeTx>) {
  return (tx.chain.values.mock.calls as unknown[][])
    ...
    .filter(
      (v): v is Record<string, unknown> => !!v && typeof v.type === 'string' && 'recipientId' in v,
    );
}
...
describe('toggleReaction notifies the content owner (#348)', () => {
  ...
  it.each([
    ...
    async (targetType, ownerId, label) => {
      ...
      await toggleReaction({ targetType, targetId: 't1', emoji: 'yum' }, reactor);
