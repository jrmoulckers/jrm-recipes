...
describe('reviewInput photoUrl (#341/#355)', () => {
  ...
  const cloudinaryUrl = 'https://res.cloudinary.com/heirloom/image/upload/v1/review.jpg';

  it('accepts an uploaded Cloudinary delivery URL', () => {
    expect(reviewInput.parse({ ...base, photoUrl: cloudinaryUrl })).toMatchObject({
  ...
  it('rejects a non-URL string', () => {
    expect(() => reviewInput.parse({ ...base, photoUrl: 'not a url' })).toThrow();
  });
});
