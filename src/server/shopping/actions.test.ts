...
describe('shopping list actions', () => {
  ...
  it('restores a bulk undo token through one server action', async () => {
    ...
    await expect(restoreShoppingListPointsAction({ restorePoints })).resolves.toEqual({
      ok: true,
      restorePoints: redoPoints,
      undoToken: { restorePoints: redoPoints },
    });
    expect(multiRestoreMock).toHaveBeenCalledOnce();
    expect(multiRestoreMock).toHaveBeenCalledWith({ id: 'user_1' }, { restorePoints });
  ...
  it('maps foreign list ids to the standard not-found action result', async () => {
    ...
    await expect(makeShoppingListDefaultAction({ listId: id('1') })).resolves.toEqual({
      ok: false,
  ...
  it('revalidates shopping after an authorized default change', async () => {
    ...
    await expect(makeShoppingListDefaultAction({ listId: id('2') })).resolves.toEqual({
      ok: true,
      defaultListId: id('2'),
    });
  ...
  it('returns the server-selected fallback after archiving', async () => {
    ...
    await expect(archiveShoppingListAction({ listId: id('1') })).resolves.toEqual({
      ok: true,
      fallbackListId: 'default',
    });
    expect(revalidatePathMock).toHaveBeenCalledWith('/shopping');
  });
  ...
  it('returns the server-selected fallback after deletion', async () => {
    ...
    await expect(deleteShoppingListAction({ listId: id('1') })).resolves.toEqual({
      ok: true,
      fallbackListId: 'default',
    });
  });
  ...
  it('unchecks all without creating a restore point', async () => {
    ...
    await expect(uncheckAllShoppingItemsAction({ listId: id('1') })).resolves.toEqual({ ok: true });
  ...
  it('restores an owned point and returns the new current-state undo id', async () => {
    ...
    expect(restorePointMock).toHaveBeenCalledWith({ id: 'user_1' }, id('1'), id('2'));
  });

  it('retrieves authorized history without revalidating', async () => {
    historyMock.mockResolvedValue([]);

    await expect(getShoppingListHistoryAction({ listId: id('1') })).resolves.toEqual({
      ok: true,
      history: [],
    });
    expect(revalidatePathMock).not.toHaveBeenCalled();
