...
const { transactionMock, getRecipeMock, groupMembersMock, mealPlanEntriesMock } = vi.hoisted(
  () => ({
...
function fakeTx() {
  ...
  const selectChain = {
    ...
    for: vi.fn(async () => {
      ...
      const params = new PgDialect().sqlToQuery(selectWheres.at(-1) as never).params;
...
describe('shopping list ownership', () => {
  it("returns NOT_FOUND before renaming another user's list", async () => {
    const tx = fakeTx();
    tx.query.shoppingLists.findFirst.mockResolvedValue(list('foreign', { userId: 'user_2' }));
    runWith(tx);
  ...
  it("returns NOT_FOUND before deleting another user's store", async () => {
    ...
    await expect(deleteShoppingStore(user, 'foreign')).rejects.toThrow('NOT_FOUND');
  ...
  it('rejects foreign move targets and alternatives as NOT_FOUND', async () => {
    ...
    foreignAlternativeTx.query.shoppingListItems.findFirst.mockResolvedValue(item);
  ...
  it('rejects foreign item and store references before saving a package route', async () => {
    ...
    foreignItemTx.query.shoppingLists.findFirst.mockResolvedValue(list('list_a'));
    ...
    expect(foreignItemTx.query.shoppingLists.findFirst).toHaveBeenCalledTimes(1);
  ...
  it('updates one owned route and persists required and purchase amounts separately', async () => {
    ...
    tx.query.shoppingLists.findFirst.mockResolvedValue(list('store', { isDefault: true }));
    tx.query.shoppingLists.findMany.mockResolvedValue([list('store', { isDefault: true })]);
...
describe('list-scoped mutations', () => {
  it('clears checked and all items only from the authorized list', async () => {
    ...
    expect(
      checkedTx.wheres.map((where) => dialect.sqlToQuery(where as never).params),
    ).toContainEqual(['list_a', true]);
    expect(allTx.wheres.map((where) => dialect.sqlToQuery(where as never).params)).toContainEqual([
      'list_b',
    ]);
  ...
  it('unchecks all in the owned list without writing history', async () => {
    ...
    expect(tx.inserted.some((write) => write.table === shoppingListRestorePoints)).toBe(false);
  });
  ...
  it('promotes a default replacement but routes to the first active alternative', async () => {
    const tx = fakeTx();
    tx.query.shoppingLists.findFirst.mockResolvedValue(list('default', { isDefault: true }));
    tx.query.shoppingLists.findMany.mockResolvedValue([
    ...
    expect(tx.sets).toContainEqual(expect.objectContaining({ preferredListId: 'alternative' }));
  });
});
...
describe('automatic ingredient routing', () => {
  it('writes each recipe contribution to exactly one destination, never an alternative', async () => {
    ...
    expect(rows).not.toContainEqual(expect.objectContaining({ listId: 'alternative' }));
  });

  it('rejects a saved preferred list that is not owned by the user', async () => {
    const tx = fakeTx();
    tx.query.shoppingLists.findMany.mockResolvedValue([list('default', { isDefault: true })]);
    tx.query.shoppingIngredientRoutes.findMany.mockResolvedValue([
      {
        id: 'route_foreign',
        userId: user.id,
    ...
    await expect(addRecipeToList(user, 'recipe_1')).rejects.toThrow('NOT_FOUND');
    expect(tx.inserted.filter((write) => write.table === shoppingListItems)).toHaveLength(0);
  });
});
...
describe('shopping restore points', () => {
  it('prunes beyond 20 deterministically in the snapshot transaction', async () => {
    ...
    expect(historyOptions.orderBy.map((order) => dialect.sqlToQuery(order as never).sql)).toEqual([
      '"shopping_list_restore_points"."createdAt" desc',
      '"shopping_list_restore_points"."id" desc',
    ]);
    expect(tx.wheres.map((where) => dialect.sqlToQuery(where as never).params)).toContainEqual([
      'list_a',
      user.id,
      'old_2',
      'old_1',
  ...
  it('restores snapshot items after recording the current state', async () => {
    ...
    await expect(restoreShoppingListPoint(user, 'list_a', 'point_1')).resolves.toEqual({
      listId: 'list_a',
      restorePointId: 'restore_1',
    });

    expect(
      tx.inserted.find((write) => write.table === shoppingListRestorePoints)?.values,
    ).toMatchObject({ operation: 'restore', listId: 'list_a' });
    expect(
      tx.inserted.find((write) => write.table === shoppingListRestorePointItems)?.values,
    ).toEqual([expect.objectContaining({ item: 'Current milk', position: 0 })]);
    expect(tx.inserted.filter((write) => write.table === shoppingListItems).at(-1)?.values).toEqual(
  ...
  it('cannot restore a foreign or stale point for an owned list', async () => {
    ...
    await expect(restoreShoppingListPoint(user, 'list_a', 'foreign_point')).rejects.toThrow(
      'NOT_FOUND',
    );
    const pointQuery = tx.query.shoppingListRestorePoints.findFirst.mock.calls[0]?.[0] as {
      where: unknown;
    };
    expect(new PgDialect().sqlToQuery(pointQuery.where as never).params).toEqual([
      'foreign_point',
      'list_a',
      user.id,
    ]);
  ...
  it('snapshots source and destination lists once for a bulk move', async () => {
    ...
    ).toEqual(['bulk_move_source', 'bulk_move_source', 'bulk_move_destination']);
  ...
  it('atomically restores every list in a bulk undo token', async () => {
    ...
    expect(
      tx.inserted
        .filter((write) => write.table === shoppingListItems)
        .map((write) => (write.values as Array<{ item: string }>).map((item) => item.item)),
    ).toEqual([['Source milk'], ['Target bread']]);
    const dialect = new PgDialect();
    expect(tx.wheres.map((where) => dialect.sqlToQuery(where as never).params)).toEqual(
      expect.arrayContaining([['list_a'], ['list_c']]),
    );
  });
  ...
  it('captures each affected list before a meal-plan rebuild', async () => {
    ...
    expect(result.restorePoints).toEqual([{ listId: 'default', restorePointId: 'restore_1' }]);
    expect(
      tx.inserted.find((write) => write.table === shoppingListRestorePoints)?.values,
