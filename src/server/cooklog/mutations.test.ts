    ...
    query: {
      recipes: {
        findFirst: vi.fn(async () =>
          opts.recipe === undefined ? { id: 'recipe_1', groupId: 'group_1' } : opts.recipe,
        ),
      },
      groupMembers: {
...
describe('createCookLog share-with-family (#352)', () => {
  it('throws NOT_FOUND when the recipe is missing', async () => {
    ...
    runWith(tx);
    await expect(
      createCookLog({ recipeId: 'nope', recipeSlug: 's', shareWithFamily: true }, cook),
  ...
  it('shares to the group when opted in and the cook is a member', async () => {
    ...
    runWith(tx);
    await createCookLog({ recipeId: 'recipe_1', recipeSlug: 's', shareWithFamily: true }, cook);
