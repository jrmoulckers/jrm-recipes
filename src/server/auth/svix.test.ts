...
describe('verifySvixSignature', () => {
  ...
  it('accepts a correctly signed payload', () => {
    ...
    expect(verifySvixSignature(SECRET, { id, timestamp, signature }, payload, now)).toBe(true);
  });
  ...
  it('rejects a tampered payload', () => {
    const signature = sign(payload, id, timestamp);
    expect(verifySvixSignature(SECRET, { id, timestamp, signature }, payload + 'x', now)).toBe(
      false,
    );
  });
  ...
  it('rejects a signature made with the wrong secret', () => {
    ...
    expect(verifySvixSignature(SECRET, { id, timestamp, signature }, payload, now)).toBe(false);
  });

  it('rejects missing headers', () => {
    const signature = sign(payload, id, timestamp);
    expect(verifySvixSignature(SECRET, { id: null, timestamp, signature }, payload, now)).toBe(
      false,
    );
    expect(verifySvixSignature(SECRET, { id, timestamp: null, signature }, payload, now)).toBe(
      false,
    );
    expect(verifySvixSignature(SECRET, { id, timestamp, signature: null }, payload, now)).toBe(
      false,
    );
  });
  ...
  it('rejects a stale timestamp outside the tolerance window', () => {
    ...
    expect(verifySvixSignature(SECRET, { id, timestamp: staleTs, signature }, payload, now)).toBe(
      false,
    );
  });
  ...
  it('rejects a non-v1 signature scheme', () => {
    const key = Buffer.from(SECRET.replace(/^whsec_/, ''), 'base64');
    const raw = createHmac('sha256', key).update(`${id}.${timestamp}.${payload}`).digest('base64');
    expect(
      verifySvixSignature(SECRET, { id, timestamp, signature: `v2,${raw}` }, payload, now),
    ).toBe(false);
  });
});
