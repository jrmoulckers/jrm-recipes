...
const KNOWN_USERS = new Set([IDS.ownerId, IDS.users.gran, IDS.users.rosa, IDS.users.mateo]);
const KNOWN_RECIPES = new Set([IDS.recipes.gravy, IDS.recipes.marinara, IDS.recipes.focaccia]);
...
describe('seed-library builders (#185)', () => {
  ...
  it('keeps collection memberships unique per (collection, recipe)', () => {
    const keys = buildCollectionRecipeRows(IDS).map((r) => `${r.collectionId}:${r.recipeId}`);
    expect(new Set(keys).size).toBe(keys.length);
  });
  ...
  it('gives the cook log believable metadata (notes, a photo, servings)', () => {
    ...
    expect(rows.every((r) => r.servingsMade == null || r.servingsMade > 0)).toBe(true);
  });
});
