...
describe('group_invitations table (issue #181)', () => {
  const { columns, indexes, checks, foreignKeys } = getTableConfig(groupInvitations);
  ...
  it('models status as an enum with the full lifecycle', () => {
    expect(col('status')?.enumValues).toEqual(['pending', 'accepted', 'revoked', 'expired']);
  ...
  it('prevents duplicate pending invites per (group, email) via a partial unique index', () => {
    const idx = indexes.find((i) => i.config.name === 'group_invitations_pending_email_uq');
  ...
  it('indexes the group foreign key', () => {
    expect(indexes.some((i) => i.config.name === 'group_invitations_group_idx')).toBe(true);
  });
  ...
  it('requires an email or handle via a CHECK', () => {
    expect(checks.some((c) => c.name === 'group_invitations_contact_check')).toBe(true);
  });
});
