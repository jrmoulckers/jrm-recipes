...
const BANS: { what: string; pattern: RegExp; probe: string }[] = [
  ...
  },
  {
    ...
    probe: 'ALTER TABLE "shopping_list_items" ADD COLUMN "required_base_quantity" real',
  },
  {
    what: 'a single-precision base-quantity-max column',
    pattern: REAL_PRECISION_COLUMN,
    probe: 'ALTER TABLE "shopping_list_items" ADD COLUMN "required_base_quantity_max" real',
  },
  {
    what: 'a single-precision purchase-quantity column',
    pattern: REAL_PRECISION_COLUMN,
    probe: 'ALTER TABLE "shopping_list_items" ADD COLUMN "purchase_quantity" real',
  },
  {
...
describe('migration bans (issue #750)', () => {
  it.each(BANS)('still matches $what, so the ban can fire', ({ pattern, probe }) => {
...
const routingMigration = readdirSync(drizzleDir)
  ...
  .find((body) => body.includes('CREATE TABLE IF NOT EXISTS "shopping_ingredient_routes"'));
...
describe('shopping routing schema (issue #630)', () => {
  it('models named, archivable store lists with one explicit default', () => {
    ...
    const active = indexes.find((item) => item.config.name === 'shopping_lists_user_active_idx');
  ...
  describe('shopping restore-point schema (issue #628)', () => {
    it('scopes restore points to both list and owner with deterministic history ordering', () => {
      const { columns, foreignKeys, indexes, checks } = getTableConfig(shoppingListRestorePoints);
      const foreignKey = (name: string) =>
        foreignKeys.find((item) =>
          item.reference().columns.some((candidate) => candidate.name === name),
        );

      expect(columns.find((column) => column.name === 'listId')?.notNull).toBe(true);
      expect(columns.find((column) => column.name === 'userId')?.notNull).toBe(true);
      expect(columns.find((column) => column.name === 'operationGroupId')?.notNull).toBe(false);
      ...
      const history = indexes.find(
        (item) => item.config.name === 'shopping_list_restore_points_list_created_idx',
      );
      expect(history && indexColumns(history)).toEqual(['listId', 'createdAt', 'id']);
      const grouped = indexes.find(
        (item) => item.config.name === 'shopping_list_restore_points_user_group_idx',
      );
      expect(grouped && indexColumns(grouped)).toEqual(['userId', 'operationGroupId']);
    });
    ...
    it('captures complete item previews with stable positions and cascading cleanup', () => {
      ...
      const pointFk = foreignKeys.find((item) =>
        item.reference().columns.some((candidate) => candidate.name === 'restorePointId'),
      );
      ...
      const orderedItems = indexes.find(
        (item) => item.config.name === 'shopping_list_restore_point_items_point_position_idx',
      );
  ...
  it('optionally links shopping lines to the canonical food graph', () => {
    ...
    expect(indexes.some((index) => index.config.name === 'shopping_list_items_food_idx')).toBe(
      true,
    );
  });
  ...
  it('stores exact requirements separately from package purchase results', () => {
    ...
    expect(column('requiredBaseQuantity')?.getSQLType()).toBe('double precision');
    expect(column('requiredBaseQuantityMax')?.getSQLType()).toBe('double precision');
  ...
  it('stores canonical and normalized fallback routes with covering indexes', () => {
    const { columns, foreignKeys, indexes } = getTableConfig(shoppingIngredientRoutes);
    const column = (name: string) => columns.find((item) => item.name === name);
    const foreignKey = (name: string) =>
      foreignKeys.find((item) =>
        item.reference().columns.some((candidate) => candidate.name === name),
    ...
    const fallback = indexes.find(
      (item) => item.config.name === 'shopping_ingredient_routes_user_normalized_item_uq',
    );
    expect(fallback?.config.unique).toBe(true);
    expect(fallback?.config.where).toBeUndefined();
    expect(fallback && indexColumns(fallback)).toEqual(['userId', 'normalizedItem']);
  });
  ...
  describe('shopping package migration (issue #629)', () => {
    it('defaults global rounding off and keeps existing rows exact', () => {
      ...
      const additiveColumns = packageMigration?.match(/^ALTER TABLE .* ADD COLUMN .*$/gm) ?? [];
      expect(additiveColumns).toHaveLength(23);
      expect(
        additiveColumns.every(
          (line) =>
            line.includes('ALTER TABLE IF EXISTS') && line.includes('ADD COLUMN IF NOT EXISTS'),
        ),
      ).toBe(true);
      expect(packageMigration?.match(/EXCEPTION WHEN duplicate_object/g)).toHaveLength(16);
  ...
  it('orders alternative lists and rejects negative positions', () => {
    ...
    expect(primaryKeys.map((key) => key.columns.map((column) => column.name))).toContainEqual([
      'routeId',
      'listId',
    ]);
    ...
    expect(
      indexes.some(
        (index) =>
          index.config.name === 'shopping_ingredient_route_alternatives_route_position_idx',
      ),
    ).toBe(true);
  });
});
...
describe('shopping routing migration (issue #630)', () => {
  it('backfills one deterministic implicit winner before enforcing uniqueness', () => {
    ...
    expect(routingMigration).toContain('HAVING count(*) FILTER (WHERE "is_default") = 0');
