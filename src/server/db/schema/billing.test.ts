...
describe('billing schema (issue #300)', () => {
  ...
  it('billing_customers maps an owner (user xor group) to a stripe customer', () => {
    const { columns, checks, uniqueConstraints, indexes } = getTableConfig(billingCustomers);
    ...
    expect(uniqueConstraints.map((u) => u.name)).toContain('billing_customers_stripe_customer_uq');
  ...
  it('usage_counters is unique per (ownerId, metric, periodStart)', () => {
    ...
    const unique = indexes.find((i) => i.config.name === 'usage_counters_owner_metric_period_uq');
