...
const migration = readdirSync(drizzleDir)
  ...
  .find((m) => m.body.includes('ADD CONSTRAINT "recipe_versions_recipe_version_uq"'));
...
describe('eng-data batch migration repairs legacy data before constraining it', () => {
  ...
  it('#151 de-duplicates version numbers before adding the UNIQUE constraint', () => {
    ...
    const unique = body.indexOf('ADD CONSTRAINT "recipe_versions_recipe_version_uq"');
  ...
  it('#170 repairs empty snapshots and casts text->jsonb with a USING clause', () => {
    const repair = body.indexOf(`UPDATE "recipe_versions" SET "snapshot" = '{}'`);
  ...
  it('#154 backfills owner-excluded rating aggregates before the CHECKs', () => {
    ...
    const countCheck = body.indexOf('ADD CONSTRAINT "recipes_rating_count_check"');
...
describe('eng-data batch migration provisions full-text search (issue #158)', () => {
  ...
  it('adds a generated, STORED search_vector weighted title>description>cuisine', () => {
    expect(body).toMatch(/ADD COLUMN "search_vector" tsvector GENERATED ALWAYS AS/);
    expect(body).toContain(`setweight(to_tsvector('english', coalesce("title", '')), 'A')`);
    expect(body).toContain(`setweight(to_tsvector('english', coalesce("description", '')), 'B')`);
    expect(body).toContain(`setweight(to_tsvector('english', coalesce("cuisine", '')), 'C')`);
