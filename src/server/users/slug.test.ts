...
function liveConflict(): Error {
  return Object.assign(
    new Error(
      'duplicate key value violates unique constraint "users_slug_unique"',
    ),
...
function aliasConflict(): Error {
  return Object.assign(
    new Error(
      'duplicate key value violates unique constraint "user_slug_aliases_pkey"',
    ),
...
function fakeTx(
  ...
  aliasFindFirst: ReturnType<typeof vi.fn>,
) {
  return {
    query: {
...
describe("isUserSlugConflict", () => {
  ...
  it("unwraps a wrapped cause", () => {
    expect(
      isUserSlugConflict(new Error("wrapped", { cause: liveConflict() })),
    ).toBe(true);
  });
...
describe("uniqueUserSlug", () => {
  ...
  it("perturbs past a reserved slug without touching the database", async () => {
    ...
    const slug = await uniqueUserSlug(
      fakeTx(userFindFirst, aliasFindFirst),
      "new",
    );
...
describe("withUserSlugConflictRetry", () => {
  it("retries a lost race and succeeds", async () => {
    const op = vi
      .fn()
      .mockRejectedValueOnce(liveConflict())
