...
export function isUserSlugConflict(err: unknown): boolean {
  ...
  if (e.code === PG_UNIQUE_VIOLATION) {
    ...
    if (name === USERS_SLUG_CONSTRAINT || name === USER_SLUG_ALIAS_CONSTRAINT)
      return true;
    if (name == null && typeof e.message === "string")
      return (
        e.message.includes(USERS_SLUG_CONSTRAINT) ||
        e.message.includes(USER_SLUG_ALIAS_CONSTRAINT)
      );
    return false;
  }
 ...
 */
export async function withUserSlugConflictRetry<T>(
  op: () => Promise<T>,
): Promise<T> {
  ...
  const alias = await tx.query.userSlugAliases.findFirst({
    where: ignoreUserId
      ? and(
          ...
          ne(userSlugAliases.userId, ignoreUserId),
        )
      : eq(userSlugAliases.slug, candidate),
    columns: { slug: true },
  });
 ...
 */
export async function uniqueUserSlug(
  tx: Db,
  base: string,
  ignoreUserId?: string,
): Promise<string> {
...
export async function changeUserSlug(
  ...
  { ok: true; slug: string } | { ok: false; reason: UserSlugRejection }
> {
  ...
  try {
    return await withUserSlugConflictRetry(() =>
      db.transaction(async (tx) => {
        ...
        if (await slugTaken(tx, normalized, userId))
...
export async function resolveUserSlug(
  ...
  if (normalized.length === 0 || normalized.length > USER_SLUG_MAX_LENGTH)
    return null;
