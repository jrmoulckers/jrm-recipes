...
describe('hideContent gating (#357)', () => {
  it('rejects a member who cannot manage the group', async () => {
    ...
    await expect(
      hideContent({ targetType: 'comment', targetId: 'comment_1', groupSlug: 'fam' }, manager),
  ...
  it("hides a target that belongs to the manager's group", async () => {
    ...
    await hideContent({ targetType: 'comment', targetId: 'comment_1', groupSlug: 'fam' }, manager);
...
describe('dismissReport gating (#357)', () => {
  it('rejects a non-manager', async () => {
    ...
    await expect(
      dismissReport({ targetType: 'comment', targetId: 'comment_1', groupSlug: 'fam' }, manager),
    ).rejects.toThrow('FORBIDDEN');
  });
