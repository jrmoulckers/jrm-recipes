...
const { transactionMock, resolveTargetMock, canViewRecipeMock, notifyManyMock } = vi.hoisted(
  () => ({
...
describe('reportContent (#356)', () => {
  ...
  it('rejects reporting your own content', async () => {
    ...
    await expect(
      reportContent({ targetType: 'comment', targetId: 'comment_1', reason: 'other' }, reporter),
  ...
  it('throws NOT_FOUND when the target no longer exists', async () => {
    ...
    await expect(
      reportContent({ targetType: 'comment', targetId: 'gone', reason: 'other' }, reporter),
