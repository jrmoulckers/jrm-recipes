...
describe('messageForError', () => {
  ...
  it('uses DEFAULT_MESSAGES for a known code with no override or fallback', () => {
    expect(messageForError(new DomainError('SELF_RATING'))).toBe(DEFAULT_MESSAGES.SELF_RATING);
  });

  it('uses the fallback for unknown/non-domain errors', () => {
    expect(messageForError(new Error('weird'), {}, 'please retry')).toBe('please retry');
  });

  it('uses a generic message for unknown errors with no fallback', () => {
    expect(messageForError(new Error('weird'))).toBe('Something went wrong. Please try again.');
  });

  it('works with legacy string-coded Errors too', () => {
    expect(messageForError(new Error('NOT_FOUND'), { NOT_FOUND: 'Gone.' })).toBe('Gone.');
  });
});
