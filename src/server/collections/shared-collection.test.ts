...
describe('getSharedCollection access rules (#281)', () => {
  ...
  it('permits an unlisted collection only when reached by its share token', async () => {
    dbMock.query.collections.findFirst.mockResolvedValue(collection({ visibility: 'unlisted' }));
    // Reached via token → allowed.
    await expect(getSharedCollection('tok_abc', stranger)).resolves.not.toBeNull();
  ...
  it('permits a public collection for anyone', async () => {
    dbMock.query.collections.findFirst.mockResolvedValue(collection({ visibility: 'public' }));
...
describe('setCollectionVisibility (#281)', () => {
  ...
  function withTx(tx: unknown) {
    dbMock.transaction.mockImplementation(async (cb: (t: unknown) => unknown) => cb(tx));
  }

  it('mints a share token the first time a collection leaves private', async () => {
    let capturedRow: Record<string, unknown> = {};
    const returning = vi.fn().mockImplementation(() => Promise.resolve([capturedRow]));
  ...
  it("throws NOT_FOUND when the caller doesn't own the collection", async () => {
    ...
    await expect(setCollectionVisibility('c1', 'public', stranger)).rejects.toThrow('NOT_FOUND');
  ...
  it('reuses an existing share token instead of minting a new one', async () => {
    const set = vi.fn().mockReturnValue({
      where: () => ({
        returning: () =>
          Promise.resolve([{ id: 'c1', visibility: 'public', shareToken: 'existing' }]),
      }),
    });
    const tx = {
      query: {
        collections: {
          findFirst: vi.fn().mockResolvedValue({ id: 'c1', shareToken: 'existing' }),
        },
      },
      update: vi.fn().mockReturnValue({ set }),
    };
