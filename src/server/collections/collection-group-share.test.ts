...
import { shareCollectionWithGroup, unshareCollectionWithGroup } from './mutations';
...
describe('getSharedCollection group sharing (#365)', () => {
  ...
  it('revokes access as soon as the share link is gone', async () => {
    dbMock.query.collections.findFirst.mockResolvedValue(collection({ sharedWithGroups: [] }));
    dbMock.query.groupMembers.findMany.mockResolvedValue([{ groupId: 'g1' }]);
...
describe('shareCollectionWithGroup / unshareCollectionWithGroup (#365)', () => {
  ...
  it("refuses to share a collection the caller doesn't own", async () => {
    ...
    await expect(shareCollectionWithGroup('c1', 'g1', stranger)).rejects.toThrow('NOT_FOUND');
  ...
  it("refuses to share with a group the owner doesn't belong to", async () => {
    ...
    await expect(shareCollectionWithGroup('c1', 'g1', owner)).rejects.toThrow('NOT_FOUND');
    expect(insert).not.toHaveBeenCalled();
  });
  ...
  it('unshares only for the owner', async () => {
    ...
    await expect(unshareCollectionWithGroup('c1', 'g1', owner)).resolves.toEqual({
      collectionId: 'c1',
      groupId: 'g1',
    });
  ...
  it("refuses to unshare a collection the caller doesn't own", async () => {
    ...
    await expect(unshareCollectionWithGroup('c1', 'g1', stranger)).rejects.toThrow('NOT_FOUND');
