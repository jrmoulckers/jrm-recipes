...
import { collectionInput, collectionRecipeInput, toggleFavoriteInput } from './validation';
...
describe('collectionInput', () => {
  ...
  it('requires a name', () => {
    expect(() => collectionInput.parse({ name: '  ' })).toThrow(/Name your collection/);
  });

  it('rejects an invalid cover image url', () => {
    expect(() => collectionInput.parse({ name: 'Soups', coverImageUrl: 'not-a-url' })).toThrow();
  });
});
...
describe('collectionRecipeInput', () => {
  it('requires both ids', () => {
    ...
    expect(() => collectionRecipeInput.parse({ collectionId: 'col1', recipeId: '' })).toThrow();
  });
});
