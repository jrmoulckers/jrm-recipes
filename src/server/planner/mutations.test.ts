...
import { addEntry, addMealWithLeftovers, copyPreviousWeek, removeEntry } from './mutations';
...
function fakeTx(opts: {
  ...
  const chain = {
    ...
    returning: vi.fn(async () =>
      created.length > 0 ? [created.shift()!] : (opts.removed ?? [{ id: 'e1' }]),
    ),
  };
  const selectChain = {
...
describe('copyPreviousWeek serving allocations (#611)', () => {
  it('copies a shared week when the caller belongs to the family', async () => {
    ...
    await expect(copyPreviousWeek(member, '2026-07-13', 'group_1')).resolves.toEqual({
      copied: 1,
      previousEmpty: false,
    });
  ...
  it('rejects a non-member copying a shared week', async () => {
    ...
    await expect(copyPreviousWeek(outsider, '2026-07-13', 'group_1')).rejects.toThrow('FORBIDDEN');
