...
describe('addEntryInput', () => {
  ...
  it('requires either a recipe or a note', () => {
    ...
    if (!result.success) {
      expect(result.error.flatten().fieldErrors.recipeId).toContain('Pick a recipe or add a note');
    }
  });
  ...
  it('rejects malformed or impossible dates', () => {
    expect(addEntryInput.safeParse({ date: '07/06/2026', slot: 'dinner', note: 'x' }).success).toBe(
      false,
    );
    expect(addEntryInput.safeParse({ date: '2026-13-01', slot: 'dinner', note: 'x' }).success).toBe(
      false,
    );
  });
});
...
describe('copyWeekInput', () => {
  it('accepts an optional family-plan scope', () => {
    expect(copyWeekInput.parse({ week: '2026-07-06', groupId: 'group_1' })).toEqual({
      week: '2026-07-06',
      groupId: 'group_1',
    });
  });
