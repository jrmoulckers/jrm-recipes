...
describe('recipeInput', () => {
  ...
  it('rejects an over-long heritage story (#377)', () => {
    expect(() => recipeInput.parse({ title: 'Long tale', story: 'x'.repeat(4001) })).toThrow();
  });
  ...
  describe('per-serving nutrition (i414)', () => {
    ...
    it('rejects negative and non-integer energy/sodium values', () => {
      expect(() => recipeInput.parse({ title: 'Bad', calories: '-1' })).toThrow();
      expect(() => recipeInput.parse({ title: 'Bad', sodiumMg: '-5' })).toThrow();
      expect(() => recipeInput.parse({ title: 'Bad', calories: '12.5' })).toThrow();
    });

    it('rejects out-of-range macronutrients', () => {
      expect(() => recipeInput.parse({ title: 'Bad', proteinGrams: '-2' })).toThrow();
      expect(() => recipeInput.parse({ title: 'Bad', fatGrams: '100001' })).toThrow();
    });
  });
  ...
  describe('structured dietary flags (i404)', () => {
    ...
    it('rejects unknown dietary tags', () => {
      expect(() => recipeInput.parse({ title: 'Bad', dietaryFlags: ['keto'] })).toThrow();
    });
  });
});
...
describe('ingredientInput', () => {
  ...
  it('rejects empty item text and invalid quantities', () => {
    expect(() => ingredientInput.parse({ item: ' ' })).toThrow(/Add an ingredient/);
  ...
  it('enforces the allowlist once Cloudinary is configured', async () => {
    ...
    try {
      ...
      expect(
        configured.safeParse({
          title: 'Beacon',
          steps: [{ instruction: 'Watch', videoUrl: 'https://evil.example/v.mp4' }],
        }).success,
      ).toBe(false);
      ...
      expect(
        configured.safeParse({
          title: 'Uploaded',
          coverImageUrl: 'https://res.cloudinary.com/demo/image/upload/heirloom/x.jpg',
          steps: [
            {
              instruction: 'Sear',
              imageUrl: 'https://res.cloudinary.com/demo/image/upload/heirloom/s.jpg',
            },
          ],
        }).success,
      ).toBe(true);
