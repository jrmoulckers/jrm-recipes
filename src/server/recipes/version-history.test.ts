...
describe('getRecipeVersions pagination (#159)', () => {
  ...
  it('defaults to the version-history page size and clamps hostile input', async () => {
    ...
    const defaultArg = dbMock.query.recipeVersions.findMany.mock.calls[0]![0] as FindManyArg;
    ...
    const clampedArg = dbMock.query.recipeVersions.findMany.mock.calls[0]![0] as FindManyArg;
  ...
  it('passes a keyset predicate when a cursor is supplied', async () => {
    ...
    const cursorArg = dbMock.query.recipeVersions.findMany.mock.calls[0]![0] as FindManyArg;
