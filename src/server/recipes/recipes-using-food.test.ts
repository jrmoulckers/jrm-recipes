...
const renderWhere = (where: SQL): string => dialect.sqlToQuery(where).sql.toLowerCase();
...
beforeEach(() => {
  ...
  dbMock.query.recipes.findMany.mockImplementation(async ({ where }: { where: SQL }) => {
    capturedWhere = where;
    return [];
  });
});
...
describe('getRecipesUsingFood. Visibility + soft-delete', () => {
  it('scopes an anonymous viewer to public, published, live recipes', async () => {
    ...
    expect(whereParams(capturedWhere!)).toEqual(expect.arrayContaining(['public', 'published']));
  ...
  it('includes group recipes when the viewer belongs to groups', async () => {
    stubFoodLookup([{ id: 'food_tomato' }]);
    dbMock.query.groupMembers.findMany.mockResolvedValue([{ groupId: 'grp_1' }]);
    await getRecipesUsingFood('food_tomato', { id: 'user_1' } as User);
    const sql = renderWhere(capturedWhere!);
