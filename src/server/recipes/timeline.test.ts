...
function sourceRecipe(overrides: Partial<AdaptationSource> = {}): AdaptationSource {
  return {
...
describe('buildAdaptationInput', () => {
  ...
  it('tolerates a source with no ingredients, steps, or tags', () => {
    const input = buildAdaptationInput(sourceRecipe({ ingredients: [], steps: [], tags: [] }));
...
describe('assembleTimeline', () => {
  function entry(id: string, kind: TimelineEntry['kind'], iso: string): TimelineEntry {
    return {
      id,
      kind,
      note: null,
  ...
  it('breaks ties on identical timestamps with a stable milestone order', () => {
    ...
    expect(ordered.map((e) => e.kind)).toEqual(['created', 'updated', 'published', 'adaptation']);
  });
