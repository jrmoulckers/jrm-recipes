...
import { clampPageSize, DISCOVER_PAGE_SIZE, nextPageOffset, toCursorPage } from './pagination';
...
describe('nextPageOffset', () => {
  ...
  it('stays in sync with the default page size', () => {
    expect(nextPageOffset(0, DISCOVER_PAGE_SIZE, DISCOVER_PAGE_SIZE)).toBe(DISCOVER_PAGE_SIZE);
    expect(
      nextPageOffset(DISCOVER_PAGE_SIZE, DISCOVER_PAGE_SIZE - 1, DISCOVER_PAGE_SIZE),
    ).toBeNull();
  });
