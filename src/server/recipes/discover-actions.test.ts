...
describe('offset clamping', () => {
  it.each([
    ...
    expect(listPublicRecipesMock).toHaveBeenCalledWith(expect.objectContaining({ offset: 0 }));
  });

  it('passes a valid positive offset through unchanged', async () => {
    await loadMorePublicRecipesAction(40);
    expect(listPublicRecipesMock).toHaveBeenCalledWith(expect.objectContaining({ offset: 40 }));
  });
});
...
describe('sort sanitization', () => {
  it("normalizes an unknown sort to 'recent' via parseRatingSort", async () => {
    ...
    expect(listPublicRecipesMock).toHaveBeenCalledWith(expect.objectContaining({ sort: 'recent' }));
  });
