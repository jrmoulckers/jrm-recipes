...
const { getCurrentUserMock, getRecipeMock, resolveNamespacedRecipeMock, resolveFlatRecipeMock } =
  vi.hoisted(() => ({
...
describe('getNamespacedRecipeForViewer', () => {
  it('returns a canonical hit without consulting the legacy fallback', async () => {
    ...
    await expect(getNamespacedRecipeForViewer('ada', 'apple-pie')).resolves.toEqual({
      user: viewer,
      recipe,
      disposition: 'canonical',
      legacySubRoute: null,
  ...
  it('passes a co-creator mirror through untouched (#668)', async () => {
    ...
    await expect(getNamespacedRecipeForViewer('john', 'apple-pie')).resolves.toEqual({
      user: viewer,
      recipe,
      disposition: 'mirror',
      legacySubRoute: null,
  ...
  it.each(['cook', 'print', 'keepsake', 'edit'])(
    ...
    async (subRoute) => {
      ...
      await expect(getNamespacedRecipeForViewer('apple-pie', subRoute)).resolves.toEqual({
        user: viewer,
        recipe,
        disposition: 'alias',
  ...
  it('does not treat an arbitrary second segment as a legacy sub-route', async () => {
    await expect(getNamespacedRecipeForViewer('apple-pie', 'nope')).resolves.toEqual({
      user: viewer,
      recipe: null,
      disposition: 'canonical',
      legacySubRoute: null,
