...
describe('parseRecipeSearch', () => {
  ...
  it('parses repeated and comma-joined facets into de-duped lists (#271)', () => {
    expect(parseRecipeSearch({ tag: ['weeknight', 'vegan,Weeknight'] }).tags).toEqual([
      'weeknight',
      'vegan',
    ]);
    expect(parseRecipeSearch({ cuisine: 'Mexican, Thai , mexican' }).cuisines).toEqual([
      'Mexican',
      'Thai',
    ]);
    expect(parseRecipeSearch({ meal: ['Breakfast', 'brunch,breakfast'] }).meals).toEqual([
      'Breakfast',
      'brunch',
    ]);
  });
...
describe('recipeSearchToParams', () => {
  ...
  it('omits the contextual default sort but keeps an explicit one (#260)', () => {
    ...
    expect(recipeSearchToParams({ q: 'chicken', sort: 'relevance' }).get('sort')).toBeNull();
    ...
    expect(recipeSearchToParams({ q: 'chicken', sort: 'top-rated' }).get('sort')).toBe('top-rated');
  });
  ...
  it('round-trips through parseRecipeSearch', () => {
    ...
    const reparsed = parseRecipeSearch(Object.fromEntries(recipeSearchToParams(original)));
    expect(reparsed).toEqual(original);
  });
});
...
describe('dietary filter (#273)', () => {
  it('parses repeated and comma-joined diets, dropping unknown values', () => {
    expect(parseRecipeSearch({ diet: ['vegan', 'gluten-free,not-a-diet'] }).diets).toEqual([
      'vegan',
      'gluten-free',
    ]);
  });
  ...
  it('normalizes case and de-dupes to canonical order', () => {
    expect(parseRecipeSearch({ diet: 'Egg-Free, vegan , VEGAN' }).diets).toEqual([
      'vegan',
      'egg-free',
    ]);
  });
