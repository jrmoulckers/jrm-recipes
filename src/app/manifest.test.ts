...
describe('manifest', () => {
  ...
  it('sources description and shortcut labels from the catalog', () => {
    ...
    const byUrl = Object.fromEntries((m.shortcuts ?? []).map((s) => [s.url, s]));
    expect(byUrl['/recipes/new']?.name).toBe(en.metadata.shortcuts.newRecipe.name);
  ...
  it('declares a POST share_target that accepts shared photos', () => {
    ...
    expect(Array.isArray(file?.accept) ? file?.accept : [file?.accept]).toContain('image/jpeg');
  });
});
