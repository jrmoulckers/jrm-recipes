...
describe('GET /api/oembed', () => {
  it('returns a rich payload for a public recipe', async () => {
    ...
    const res = await get(`?url=${encodeURIComponent(`${ORIGIN}/recipes/apple-pie`)}&format=json`);
  ...
  it("resolves a canonical namespaced url through the cook's namespace", async () => {
    ...
    const res = await get(`?url=${encodeURIComponent(`${ORIGIN}/recipes/ada/apple-pie`)}`);
    expect(res.status).toBe(200);
  ...
  it('404s when a namespaced url resolves to nothing', async () => {
    ...
    const res = await get(`?url=${encodeURIComponent(`${ORIGIN}/recipes/ada/ghost`)}`);
  ...
  it('501s for non-json formats', async () => {
    const res = await get(`?url=${encodeURIComponent(`${ORIGIN}/recipes/apple-pie`)}&format=xml`);
  ...
  it('404s when the recipe is not public', async () => {
    ...
    const res = await get(`?url=${encodeURIComponent(`${ORIGIN}/recipes/secret`)}`);
    expect(res.status).toBe(404);
  });
});
