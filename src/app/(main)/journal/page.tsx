...
async function JournalPage({
  ...
  const selectedRecipeId =
    recipeParam && recipeOptions.some((option) => option.id === recipeParam) ? recipeParam : null;
  ...
  const [cooks, insights] =
    dbReady && user
      ? await Promise.all([getFilteredCooks(user.id, filter), getJournalInsights(user.id, filter)])
      : [
          [],
          {
            totalCooks: 0,
...
async function InsightsStrip({ insights, locale }: { insights: JournalInsights; locale: string }) {
          ...
          <ol className="flex flex-col gap-1">
            {insights.topRecipes.map((recipe) => (
              <li key={recipe.id} className="flex items-center justify-between gap-2 text-sm">
                ...
                <span className="shrink-0 text-muted-foreground">×{recipe.count}</span>
              </li>
            ))}
          </ol>
        ) : (
