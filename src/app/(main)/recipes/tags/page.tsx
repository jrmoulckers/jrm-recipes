...
import { ArrowLeft, ChefHat, Globe2, Leaf, Tags, TrendingUp, type LucideIcon } from 'lucide-react';
  ...
  return (
    <div className="container flex flex-col gap-8 py-10">
      <div className="flex flex-col gap-3">
        ...
        <p className="text-muted-foreground">{tags.length > 0 ? t('intro') : t('introEmpty')}</p>
      </div>
        ...
        <>
          <section className="flex flex-col gap-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-display text-xl font-bold tracking-tight">{t('popular')}</h2>
            </div>
  ...
  return (
    <section className="flex flex-col gap-6">
      <h2 className="font-display text-xl font-bold tracking-tight">
        {t('allTags')}{' '}
        <span className="text-base font-normal text-muted-foreground">({tags.length})</span>
      </h2>
      {TAG_CATEGORIES.filter((category) => groups.has(category)).map((category) => {
