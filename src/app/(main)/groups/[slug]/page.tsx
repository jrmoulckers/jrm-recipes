...
import { getRecentCookAlongsToLog, getUpcomingCookAlongs } from '~/server/cookalong/queries';
...
import { MemberList, type MemberListMember } from '~/components/groups/member-list';
...
export async function generateMetadata({
  ...
  const description = group.description ?? tMeta('group.description', { brand: brand.name });
...
async function GroupPage({ params }: { params: Promise<SlugRouteParams> }) {
  ...
  const openReportCount = canManage ? await getOpenReportCount(group.id, group.viewerRole) : 0;
  const activity =
  ...
  const sharedCollections =
    isMember && viewer ? await listCollectionsSharedWithGroup(group.id, viewer) : [];
  ...
  return (
    <div className="container flex flex-col gap-8 py-10">
      <Breadcrumbs items={[{ label: tNav('family'), href: '/groups' }, { label: group.name }]} />
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          <div className="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-start">
            ...
            <div className="min-w-0">
              ...
              {group.description ? (
                <p className="mt-3 max-w-2xl text-lg text-muted-foreground">{group.description}</p>
              ) : (
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_22rem]">
          ...
          <section className="flex flex-col gap-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-muted-foreground">{t('members.body')}</p>
              </div>
              {canManage ? <InviteLinkManager slug={group.slug} /> : null}
            </div>
            {canManage ? <AddMemberForm slug={group.slug} /> : null}
            {isMember ? (
              <MemberList slug={group.slug} viewerRole={group.viewerRole} members={members} />
            ) : (
          ...
          <section className="flex flex-col gap-4">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-muted-foreground">{t('cookbook.body')}</p>
              </div>
              {group.viewerRole ? (
                <Button asChild variant="outline">
            ...
            <>
              ...
              <section className="flex flex-col gap-4">
                <div>
                  ...
                  <p className="mt-1 text-muted-foreground">{t('shared.body')}</p>
                </div>
                <div className="grid gap-5 sm:grid-cols-2">
        ...
        <aside className="flex flex-col gap-4 lg:sticky lg:top-20 lg:self-start">
          <div className="rounded-2xl border border-border bg-card p-5 shadow-token">
            <h2 className="font-display text-xl font-semibold">{t('tools.heading')}</h2>
        ...
        {recipe.author?.name ? (
          <p className="mt-3 text-xs text-muted-foreground">
            {t.rich('sharedBy', {
              ...
              author: (chunks) => <span className="font-medium text-foreground">{chunks}</span>,
            })}
          </p>
        ) : null}
      </div>
