...
import { getLimitStatus, getSubscriptionSnapshot } from '~/server/billing/entitlements';
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-5">
          <p className="flex items-baseline gap-1">
            <span className="font-display text-2xl font-bold">${plan.monthlyPriceUsd}</span>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button asChild variant={isPaid ? 'ghost' : 'default'}>
              <Link href="/pricing">
                {isPaid ? t('viewAllPlans') : t('upgrade', { plan: tPlan('family.name') })}
              </Link>
            </Button>
          </div>

          {isPaid && !billingReady ? (
            <p className="text-sm text-muted-foreground">{t('billingUnavailable')}</p>
          ) : null}
        </CardContent>
      </Card>
