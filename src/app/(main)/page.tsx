...
import { getOnboardingProgress, isOnboardingComplete } from '~/server/onboarding/progress';
...
async function loadPersonalizedHome(user: User) {
  ...
  const [dinner, rotation, quickPlan, library, myGroups, activity, following] = await Promise.all([
...
async function HomePage() {
  ...
  const [personalized, onboarding] =
    user && isDbConfigured()
      ? await Promise.all([loadPersonalizedHome(user), getOnboardingProgress(user)])
      : [null, null];
  ...
  const showRotation = personalized != null && personalized.rotation.length >= ROTATION_MIN;
  ...
  const greeting = firstName ? t('greeting.named', { name: firstName }) : t('greeting.generic');
          ...
          <div className="container flex flex-col gap-5 py-10">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-muted-foreground">{t('greeting.body')}</p>
              </div>
      ...
      {personalized && (showDinner || showRotation) && (
        <section className="border-b border-border bg-surface">
          <div className="container flex flex-col gap-10 py-10">
            {showDinner && (
              <DinnerSuggestion candidates={personalized.dinner} today={personalized.today} />
            )}
            {showRotation && (
              <RotationRail recipes={personalized.rotation} quickPlan={personalized.quickPlan} />
            )}
          </div>
        </section>
      )}
      {personalizedHome && personalized && (
        <section className="container py-12">
          <div className="mb-5 flex items-end justify-between gap-3">
            <h2 className="font-display text-2xl font-bold tracking-tight">{t('yourRecipes')}</h2>
        ...
        <section className="container py-12">
          ...
          <div className="max-w-2xl">
            <ActivityFeedLazy
              ...
              emptyState={personalized.hasGroups ? undefined : <FamilyFeedEmptyNudge />}
            />
          </div>
        </section>
      )}
        ...
        <section className="container py-12">
          <div className="mb-5 flex items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-muted-foreground">{t('following.body')}</p>
            </div>
          </div>
          <div className="max-w-2xl">
            <ActivityFeedLazy
              ...
              {features.map((f) => (
                <Card key={f.id} interactive className="h-full">
                  <CardContent className="flex flex-col gap-3 p-6">
                    ...
                    <p className="text-sm text-muted-foreground">{t(`features.${f.id}.body`)}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
            ...
            <Card className="overflow-hidden border-primary/20 bg-primary/5">
              <CardContent className="flex flex-col items-center gap-5 p-10 text-center">
                ...
                <div className="mt-2 w-full max-w-md">
                  <p className="mb-3 text-sm text-muted-foreground">{t('closing.waitlist')}</p>
