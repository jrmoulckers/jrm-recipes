...
import { getShoppingListHistory, getShoppingWorkspace } from '~/server/shopping/queries';
...
async function ShoppingPage({
  ...
  const items: ShoppingViewItem[] = (workspace?.selectedList?.items ?? []).map((row) => {
    ...
    return {
      ...
      routeAlternativeListIds:
        route?.alternativeListIds.filter((id) =>
          workspace?.lists.some((list) => list.id === id && list.archivedAt == null),
        ) ?? [],
  ...
  return (
    <div className="container flex max-w-3xl flex-col gap-8 py-10">
      ...
      {dbEnabled ? (
        ...
        <LocalShoppingList selectedListId={selectedListId} />
      )}
    </div>
  );
}
