...
const layoutSrc = readFileSync(resolve(process.cwd(), 'src/app/layout.tsx'), 'utf8').replace(
  /'/g,
  '"',
);
const themesCss = readFileSync(resolve(process.cwd(), 'src/styles/themes.css'), 'utf8').replace(
  /'/g,
  '"',
);
...
function fontOptions(constructor: string): string {
  const match = new RegExp(`${constructor}\\(\\{([\\s\\S]*?)\\}\\)`).exec(layoutSrc);
...
describe('theme font loading (#182)', () => {
  ...
  it('gives every family a size-matched fallback + adjustFontFallback', () => {
    for (const constructor of ['Fraunces', 'Nunito', 'Inter', 'Baloo_2', 'JetBrains_Mono']) {
      const opts = fontOptions(constructor);
      expect(opts, `${constructor} needs a fallback stack`).toContain('fallback:');
      expect(opts, `${constructor} needs adjustFontFallback`).toContain('adjustFontFallback');
    }
  });

  it('declares every font variable the theme CSS actually paints', () => {
    const referenced = new Set(
      Array.from(themesCss.matchAll(/var\(--font-([a-z]+)\)/g)).map((m) => m[1]),
    );
