...
describe('kid-copy', () => {
  it('returns the fallback when kidSafe is off', () => {
    expect(pickKidCopy(false, 'cta.create', 'Create a recipe')).toBe('Create a recipe');
  });

  it('returns the kid variant when kidSafe is on', () => {
    expect(pickKidCopy(true, 'cta.create', 'Create a recipe')).toBe(KID_COPY['cta.create']);
  });
  ...
  it('maps the active theme to kidSafe behavior', () => {
    ...
    expect(pickCopy('kitchen', 'empty.recipes.title', 'No recipes yet')).toBe('No recipes yet');
  });
