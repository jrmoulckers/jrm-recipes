...
describe('ios splash config', () => {
  ...
  it('builds device-scoped media queries', () => {
    ...
    expect(iosSplashMedia(device, 'portrait')).toContain('(-webkit-device-pixel-ratio: 3)');
    expect(iosSplashMedia(device, 'landscape')).toContain('(orientation: landscape)');
  });

  it('names files deterministically', () => {
    const device = { w: 768, h: 1024, dpr: 2, label: 'test' };
    expect(iosSplashFileName(device, 'portrait')).toBe('apple-splash-768-1024-2x-portrait.png');
  });
});
