...
const addMealWithLeftoversAction = vi.fn<(input: unknown) => Promise<ActionResult>>();
...
vi.mock('~/server/planner/actions', () => ({
  ...
  addMealWithLeftoversAction: (input: unknown) => addMealWithLeftoversAction(input),
}));
...
describe('PlannerBoard. Cooked it (#422)', () => {
  it('shows a Cooked it action for recipe entries', () => {
    ...
    expect(screen.getByRole('button', { name: /cooked it/i })).toBeInTheDocument();
  });

  it('does not show Cooked it for note-only entries', () => {
    render(<PlannerBoard days={days} entries={[noteEntry]} recipes={[]} />);
    expect(screen.queryByRole('button', { name: /cooked it/i })).not.toBeInTheDocument();
  });
...
describe('PlannerBoard. Batch cook / leftovers (#380)', () => {
  ...
  it('marks the primary with a batch badge and the linked night as leftovers', () => {
    render(<PlannerBoard days={weekDays} entries={[primary, leftovers]} recipes={[]} />);
  ...
  it('removing a batch primary offers to also remove the leftovers', async () => {
    ...
    await waitFor(() => expect(removeEntryAction).toHaveBeenCalledWith({ entryId: 'entry-1' }));
  ...
  it('plans exact servings across any source and destination meal', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /add to lunch on monday, jul 6/i }));
  ...
  it('only offers leftover meals after the source meal', async () => {
    ...
    expect(screen.queryByRole('option', { name: /tuesday, jul 7/i })).toBeNull();
    expect(screen.getByRole('option', { name: /thursday, jul 9/i })).toBeInTheDocument();
  ...
  it('prefills the latest serving count ahead of the recipe default', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /add to breakfast on monday/i }));
    fireEvent.click(await screen.findByRole('button', { name: /weeknight chili/i }));

    expect(screen.getByLabelText(/servings for this meal/i)).toHaveValue(6);
    expect(screen.getByText(/you made 6 servings last time/i)).toBeInTheDocument();
  });
});
...
describe('PlannerBoard. Serving allocations (#611)', () => {
  ...
  it('shows total servings on the cook and servings on every destination', () => {
    render(<PlannerBoard days={weekDays} entries={[source, lunch, dinner]} recipes={[]} />);
  ...
  it('removes every linked allocation through the source meal', async () => {
    ...
    fireEvent.click(await screen.findByRole('button', { name: /remove all 3 meals/i }));
