...
export function AnchoredSuggestions({
  ...
  const suggestionsList = hasSuggestions ? (
    <ul className={cn('mb-1.5 flex flex-col gap-1.5', compactTrigger && 'col-span-3 ms-9 mt-1.5')}>
  ...
  const interaction = canInteract ? (
    open ? (
      <form
        onSubmit={submit}
        className={cn('flex flex-col gap-2', compactTrigger && 'col-span-3 ms-9 mt-1.5')}
      >
        <Textarea
          value={body}
      ...
      <button
        ...
        aria-label={compactTrigger ? t('triggerAria', { label: anchorLabel }) : undefined}
