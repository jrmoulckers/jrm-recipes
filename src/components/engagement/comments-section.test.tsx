...
import { addCommentAction, applySuggestionAction } from '~/server/engagement/actions';
...
describe('CommentsSection', () => {
  ...
  it('labels suggestions distinctly from plain comments', () => {
    ...
    expect(within(screen.getByRole('list')).getByText('Suggestion')).toBeInTheDocument();
  });
  ...
  it('prompts sign-in and hides the form when the viewer cannot post', () => {
    ...
    expect(
      screen.getByText('Sign in to add a comment, reply, or suggest a recipe change.'),
    ).toBeInTheDocument();
    expect(
  ...
  it('offers accept and apply to the recipe owner on an open suggestion', () => {
    ...
    expect(screen.getByRole('button', { name: /accept and apply/i })).toBeInTheDocument();
  });
  ...
  it('hides accept and apply from non-owners', () => {
    ...
    expect(screen.queryByRole('button', { name: /accept and apply/i })).not.toBeInTheDocument();
  });
  ...
  it('hides accept and apply once a suggestion is applied and shows an Applied badge', () => {
    ...
    expect(screen.queryByRole('button', { name: /accept and apply/i })).not.toBeInTheDocument();
    expect(within(screen.getByRole('list')).getByText('Applied')).toBeInTheDocument();
  });
