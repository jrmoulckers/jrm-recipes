...
import { type LocalShoppingList as LocalList, useShoppingStore } from '~/lib/shopping-store';
...
describe('LocalShoppingList URL navigation', () => {
  ...
  it('honors an initial deep link and persists the viewed list', async () => {
    ...
    expect(await screen.findByTestId('current-list')).toHaveTextContent('warehouse');
    await waitFor(() => expect(useShoppingStore.getState().currentListId).toBe('warehouse'));
  ...
  it.each([undefined, '', 'missing', 'archived', 'bad/id'])(
    ...
    async (selectedListId) => {
      ...
      expect(await screen.findByTestId('current-list')).toHaveTextContent('default');
      await waitFor(() =>
  ...
  it('follows Back and Forward URL changes without adding history entries', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('current-list')).toHaveTextContent('warehouse'));
    expect(useShoppingStore.getState().currentListId).toBe('warehouse');

    view.rerender(<LocalShoppingList selectedListId="default" />);
    await waitFor(() => expect(screen.getByTestId('current-list')).toHaveTextContent('default'));
    expect(useShoppingStore.getState().currentListId).toBe('default');
  ...
  it('pushes a newly created list without making it the default', async () => {
    ...
    expect(useShoppingStore.getState().currentListId).toBe('00000000-0000-4000-8000-000000000001');
    expect(useShoppingStore.getState().defaultListId).toBe('default');
  });
  ...
  it.each([['Archive', 'archive'] as const, ['Delete', 'delete'] as const])(
    ...
    async (actionLabel, state) => {
      ...
      expect(useShoppingStore.getState().currentListId).toBe('default');
      expect(useShoppingStore.getState().defaultListId).toBe('default');
      const warehouse = useShoppingStore.getState().lists.find((list) => list.id === 'warehouse');
      if (state === 'archive') {
