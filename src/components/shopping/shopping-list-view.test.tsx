...
describe('ShoppingListView localization', () => {
  it.each(localizedCases)(
    ...
    ({ locale, messages, input, category, renderedQuantity, direction }) => {
      ...
      render(
        <div data-testid="localized-shopping-root" dir={localeDirection(locale)}>
          <ShoppingListView
            items={[
              {
                ...item,
      ...
      expect(screen.getByTestId('localized-shopping-root')).toHaveAttribute('dir', direction);
...
describe('ShoppingListView routing controls', () => {
  it('shows alternatives without duplicating an item and moves by keyboard-accessible controls', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Move Milk to another list' }));
  ...
  it('previews and restores a recent list state', () => {
    ...
    expect(onRestoreHistory).toHaveBeenCalledWith(expect.objectContaining({ id: 'point-1' }));
...
describe('ShoppingListView package guidance', () => {
  ...
  it('preserves an exact non-package quantity when no conversion is valid', () => {
    ...
    expect(within(dialog).getByLabelText('Package rounding')).toHaveValue('disable');
  });
  ...
  it('edits package, rounding, label, and preferred store in one route form', async () => {
    ...
    fireEvent.change(within(dialog).getByLabelText('Brand or package label (optional)'), {
      target: { value: 'carton' },
    });
