...
describe("ShoppingListExportMenu", () => {
  it("offers explicit export choices and hides unsupported native share", async () => {
    ...
    render(
      <ShoppingListExportMenu items={items} list={list} disabled={false} />,
      { wrapper: IntlWrapper },
    );
    ...
    expect(
      screen.getByRole("menuitem", { name: "Download text" }),
    ).toBeVisible();
    ...
    expect(
      screen.queryByRole("menuitem", { name: "Native share" }),
  ...
  it("shows native share only when capability detection succeeds", async () => {
    ...
    expect(
      await screen.findByRole("menuitem", { name: "Native share" }),
    ).toBeVisible();
  });
