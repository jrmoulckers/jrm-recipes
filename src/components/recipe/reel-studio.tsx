 ...
 * and scene builders load as a separate chunk only when the dialog opens (#200).
 *
 * It renders the *inner* of the dialog (the parent owns `<Dialog>`/`<DialogContent>`
 * and the trigger). Because it mounts only while the dialog is open, image
 * preloading runs on mount and its teardown runs on unmount. `busyRef` lets the
 * parent block closing mid-render without importing any of this chunk.
 */
export function ReelStudio({
  ...
  React.useEffect(() => {
    ...
    return stopPreview;
  }, [state, reducedMotion, busy, exportMode, scenes, stopPreview]);
