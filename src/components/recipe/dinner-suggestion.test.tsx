...
describe('DinnerSuggestion (#375)', () => {
  it('prompts to create/browse when there are no candidates', () => {
    ...
    expect(screen.queryByRole('button', { name: /pick my dinner/i })).not.toBeInTheDocument();
  ...
  it('reveals one recipe with Cook + add-to-tonight actions', async () => {
    ...
    expect(cook.getAttribute('href')).toMatch(/\/recipes\/nonna\/(tacos|pasta)\/cook$/);

    await user.click(screen.getByRole('button', { name: /add to tonight's plan/i }));
    ...
    expect(await screen.findByRole('button', { name: /on tonight's plan/i })).toBeInTheDocument();
  });
  ...
  it("hides 'pick again' when there is only one candidate", async () => {
    const user = userEvent.setup();
    render(<DinnerSuggestion candidates={[candidate('solo', 'Solo stew')]} today="2026-07-06" />);
    await user.click(screen.getByRole('button', { name: /pick my dinner/i }));
    expect(screen.queryByRole('button', { name: /pick again/i })).not.toBeInTheDocument();
  });
