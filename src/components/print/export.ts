...
function groupBySection<T extends { section: string | null }>(items: T[]): SectionGroup<T>[] {
...
export function groupSteps(steps: PrintRecipeStep[]): SectionGroup<PrintRecipeStep>[] {
...
export function formatRecipeMeta(recipe: PrintRecipe): string[] {
  ...
  if (recipe.servings != null) {
    meta.push(`${formatQuantity(recipe.servings)} ${recipe.servingsNoun ?? 'servings'}`);
  }
...
export function formatIngredientAmount(ingredient: PrintRecipeIngredient): string {
  const unit = displayUnit(ingredient.unit, ingredient.quantityMax ?? ingredient.quantity);
...
export function formatIngredientLine(ingredient: PrintRecipeIngredient): string {
  const amount = formatIngredientAmount(ingredient);
  const note = ingredient.note ? `. ${ingredient.note}` : '';
  const optional = ingredient.optional ? ' (optional)' : '';

  return cleanLine(`${amount ? `${amount} ` : ''}${ingredient.item}${note}${optional}`);
}

export function formatStepLine(step: PrintRecipeStep): string {
  const details = [
    step.timerSeconds != null ? `Timer: ${formatTimerLabel(step.timerSeconds)}` : null,
    step.techniques && step.techniques.length > 0
      ? `Techniques: ${step.techniques.join(', ')}`
      : null,
  ].filter((detail): detail is string => detail != null);

  return cleanLine(`${step.instruction}${details.length > 0 ? ` (${details.join('; ')})` : ''}`);
}
