...
export function RecipePrintBody({ recipe }: { recipe: PrintRecipe }) {
  ...
  return (
    <div>
      <h2 className="font-display text-2xl font-bold tracking-tight">{recipe.title}</h2>
      {recipe.description ? (
      ...
      <section className="mt-6">
        <h3 className="font-display text-lg font-semibold">{t('ingredients')}</h3>
                ...
                {group.items.map((step) => {
                  ...
                  return (
                    <li key={step.id} className="flex gap-3">
                      <span className="font-display font-semibold text-primary">{stepNumber}.</span>
      ...
      {recipe.notes ? (
        <section className="mt-6">
          ...
          <p className="mt-1 whitespace-pre-line text-muted-foreground">{recipe.notes.trim()}</p>
        </section>
      ) : null}
      ...
      {recipe.story || provenance.length > 0 ? (
        <section className="mt-6">
          ...
          {recipe.story ? (
            <p className="mt-2 whitespace-pre-line text-muted-foreground">{recipe.story.trim()}</p>
          ) : null}
        </section>
      ) : null}
    </div>
