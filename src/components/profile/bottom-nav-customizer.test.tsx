...
describe('BottomNavCustomizer', () => {
  it('lists pinned tabs with keyboard-operable reorder controls', () => {
    ...
    expect(within(dialog).getByRole('button', { name: 'Move Recipes later' })).toBeInTheDocument();
  });

  it('moves a tab later and updates the store order', () => {
    const dialog = openDialog();
    fireEvent.click(within(dialog).getByRole('button', { name: 'Move Home later' }));
    expect(useBottomNavStore.getState().pinned).toEqual(['recipes', 'home', 'plan', 'shopping']);
  });
  ...
  it('removes and adds tabs', () => {
    ...
    fireEvent.click(within(dialog).getByRole('button', { name: 'Add Discover' }));
