...
describe('Cook Mode chrome safe-area insets (issue #283)', () => {
  it('pads the sticky footer so Previous/Next/Done clear the home indicator and side notch', () => {
    ...
    expect(footer?.className).toContain('pl-[max(0.75rem,env(safe-area-inset-left))]');
    expect(footer?.className).toContain('pr-[max(0.75rem,env(safe-area-inset-right))]');
    expect(footer?.className).toContain('sm:pl-[max(1.25rem,env(safe-area-inset-left))]');
    expect(footer?.className).toContain('sm:pr-[max(1.25rem,env(safe-area-inset-right))]');
  });
  ...
  it('pads the sticky header for the status bar and side notch', () => {
    ...
    expect(row?.className).toContain('sm:pr-[max(1.25rem,env(safe-area-inset-right))]');
  });

  it('gives EmptyCookExperience the same top and side insets', () => {
    const { container } = render(<CookExperience recipe={makeRecipe({ steps: [] })} />);
    const header = container.querySelector('header');
    const row = header?.querySelector('div');
...
describe('Cook Mode kid-safety callout (issue #423)', () => {
  ...
  it('warns to ask a grown-up on a hot step in Kids mode', () => {
    ...
    expect(screen.getByRole('note', { name: /this step is hot/i })).toBeInTheDocument();
  });
...
describe('Cook Mode get-ready gate (issue #444)', () => {
  ...
  it("shows the 'Let's get ready!' checklist before step 1 in Kids mode", () => {
    ...
    expect(screen.getByRole('heading', { name: /let's get ready/i })).toBeInTheDocument();
  ...
  it('proceeds into step 1 when the child taps the ready button', () => {
    ...
    expect(screen.queryByRole('heading', { name: /let's get ready/i })).toBeNull();
  });
...
describe("Cook Mode 'Read it to me' narration (issue #411)", () => {
  ...
  it('reads the current step aloud and toggles to a stop control', () => {
    ...
    expect(screen.getByRole('button', { name: /read it to me/i })).toBeInTheDocument();
  });
...
describe('Cook Mode Kids countdown ring (issue #442)', () => {
  ...
  it('renders the depleting ring and keeps the digital readout in Kids mode', () => {
    ...
    expect(container.querySelector('[data-testid="kids-timer-ring"]')).not.toBeNull();
...
describe('Cook Mode Kids step trail (issue #441)', () => {
  ...
  it('shows a tappable step trail that navigates in Kids mode', () => {
    ...
    expect(screen.getByRole('button', { name: 'Go to step 1 of 3' })).toBeInTheDocument();
    expect(screen.queryByRole('progressbar')).toBeNull();

    fireEvent.click(screen.getByRole('button', { name: 'Go to step 3 of 3' }));
    expect(screen.getByRole('heading', { name: 'Let it cool.' })).toBeInTheDocument();
  });
  ...
  it('keeps the plain progress bar in grown-up modes', () => {
    ...
    expect(screen.queryByRole('button', { name: 'Go to step 1 of 3' })).toBeNull();
  });
});
...
describe('Cook Mode completion moment (issue #437)', () => {
  /* jsdom doesn't implement the object-URL helpers, so we save/restore them by
     ...
     reassigned, never invoked detached. */
