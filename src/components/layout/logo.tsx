...
export function LogoMark({ className, ...props }: React.SVGProps<SVGSVGElement>) {
  return (
    ...
    >
      {/* sprig */}
      <path d="M16 12c0-3 1.6-5.2 4-6.4-.2 2.9-1.3 5-4 6.4Z" className="fill-secondary" />
      <path d="M16 12c0-2.6-1.4-4.6-3.8-5.7.1 2.6 1.2 4.5 3.8 5.7Z" className="fill-secondary/70" />
      {/* pot body */}
      <path
      ...
      />
      {/* rim + handles */}
      <rect x="4.5" y="10.5" width="23" height="3.4" rx="1.7" className="fill-accent" />
...
export function Logo({
  ...
  return (
    <span className={cn('inline-flex items-center gap-2', className)}>
      ...
      {showWordmark && (
        <span
          className={cn('font-display text-xl font-semibold tracking-tight', wordmarkClassName)}
        >
          {brand.name}
        </span>
      )}
