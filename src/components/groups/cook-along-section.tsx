...
import { createCookAlongAction, rsvpCookAlongAction } from '~/server/cookalong/actions';
import type { PastCookAlongPrompt, UpcomingCookAlong } from '~/server/cookalong/queries';
    ...
    <section className="flex flex-col gap-4" aria-label={t('a11y.section')}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {isMember && recipes.length > 0 ? (
          <ScheduleCookAlongButton groupSlug={groupSlug} groupId={groupId} recipes={recipes} />
        ) : null}
      </div>
            ...
            >
              ...
              {event.recipe ? (
                <Button asChild size="sm" variant="outline">
                  <Link href={`/recipes/${event.recipe.slug}`}>{t('logPrompt.action')}</Link>
                </Button>
              ) : null}
            </li>
          ))}
      ...
      {upcoming.length > 0 ? (
        <ul className="flex flex-col gap-3">
          {upcoming.map((event) => (
            <CookAlongCard key={event.id} groupSlug={groupSlug} event={event} canRsvp={isMember} />
          ))}
        </ul>
      ) : (
        <div className="rounded-xl border border-dashed border-border bg-surface/50 p-6 text-center text-sm text-muted-foreground">
          <CalendarClock className="mx-auto mb-2 size-6 text-muted-foreground" aria-hidden="true" />
          {t('empty.title')}
...
function CookAlongCard({
  ...
  return (
    <li className="flex flex-col gap-4 rounded-xl border border-border bg-card p-5 shadow-token">
        ...
        <div className="flex items-center gap-2">
          <div className="flex -space-x-2">
            {going.slice(0, 6).map((attendee) => (
              <Avatar key={attendee.userId} className="size-7 border-2 border-card">
                {attendee.user?.avatarUrl ? (
                  <AvatarImage src={attendee.user.avatarUrl} alt={attendee.user.name ?? ''} />
                ) : null}
                <AvatarFallback className="text-xs">
                  {initial(attendee.user?.name ?? null, attendee.user?.handle ?? null)}
                </AvatarFallback>
              </Avatar>
            ))}
          </div>
      ...
      {canRsvp ? (
        <RsvpControls groupSlug={groupSlug} cookAlongId={event.id} current={event.viewerStatus} />
      ) : null}
    </li>
  );
}
...
function RsvpControls({
  ...
  const [optimistic, setOptimistic] = React.useState<RsvpStatus | null>(current);
