...
| D1 / D7 / W4 returning-cook retention | Share of signup-week cohorts with at least one `cook_started` in the D1, D7, or W4 window, per `docs/analytics/retention.md`.                                                         | `signup_completed`, `cook_started`, and `householdId` for household view                                           | Shows whether cooking becomes a habit.                                                                     |
...
| Share and virality                    | Weekly counts of recipe shares, share-link copies, and reel exports.                                                                                                                  | `recipe_shared`, `share_link_copied`, `reel_exported`                                                              | Sharing creates referral and emotional-value loops.                                                        |
...
| Experiment exposure and conversion    | Exposure-based conversion by flag value.                                                                                                                                              | `$feature_flag_called`, plus metric event such as `first_recipe_created`                                           | Keeps activation experiments tied to real outcomes. See `docs/analytics/experiments/empty-library-cta.md`. |
