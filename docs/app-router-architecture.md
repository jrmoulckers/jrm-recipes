...
- A recipe with co-creators resolves under **every** accepted creator's namespace as well as its owner's (#668). `resolveNamespacedRecipe()` returns a `disposition` rather than a `canonical` boolean, because a creator path is neither canonical nor a redirect: it is a `mirror`, rendered **200** with `rel=canonical` pointing at the owner's path. A 308 would take a creator off the URL they shared; omitting the canonical tag would split the SEO signal across N addresses. The boolean was replaced rather than supplemented so the third case cannot be silently folded into one of the other two. `sitemap.ts` submits owner paths only — listing mirrors would offer URLs the canonical tag immediately asks the crawler to discard.
- A mirror reached through a _retired_ user slug degrades to `alias` and 308s to the owner's canonical path. The address is already non-canonical, so there is nothing to preserve by rendering it in place.
...
- Recipe path revalidation must **fan out**, because a recipe is served at its canonical `/recipes/<cook>/<slug>` URL, at the flat legacy one, and at one path per accepted co-creator (#668) — and the App Router caches those independently. [`src/server/recipes/revalidate.ts`](../src/server/recipes/revalidate.ts) owns that: `revalidateRecipePaths()` when the caller holds the cook slug, and `revalidateRecipeSlugPaths()` for the engagement/collection/cook-log actions whose client only holds a recipe slug. Since slugs are unique per cook rather than globally, the latter busts the canonical path of _every_ namespace holding that slug — including one held by a co-creator entry rather than a recipe row. Over-revalidating is harmless, whereas missing the right owner leaves a stale page.
- Creator paths are discovered inside `revalidateRecipePaths()` rather than passed in, so no caller can forget half the fan-out. Its `extraCreators` argument exists for the one case discovery cannot reach: a namespace that has _just stopped_ resolving because the row was deleted. Purging it is the cache half of revocation — skip it and a removed co-creator's page keeps being served after their access was withdrawn.
