...
were reconciled with the merged schema/authorization, analytics, and account-bound cleanup
controls. That engineering verification does not satisfy the production-enable human gates in this
document.
...
- The current server exposes no profile/custom model-result submission route. The custom-restriction
  input contract requires the literal `self` declaration, custom-restriction corrections enforce
  the same gate, and canonical promotion rejects profile/custom targets. A future write route must
  reuse this server contract rather than infer scope from client state.
...
| Model asset and runtime state                         | Static model bytes, integrity metadata, worker state, capability tier, and a one-way local account-owner marker                      | Run optional interpretation locally and prevent cross-account cache reuse                   | Model bytes are not personal data; request metadata, the hashed owner marker, and account-bound analysis state can be | Selected model distributor or Vercel for asset delivery; browser Cache Storage/IndexedDB/memory/localStorage     | Distributor receives request metadata only. No recipe or dietary content. The local owner marker never enters telemetry                                                                                   | Purge account-bound artifacts on disable, sign-out, account switch, and deletion completion; maximum 30-day cache age                                                |
| Dietary operational analytics                         | Coarse enablement, model download outcome, capability tier, analysis outcome, trigger, and fixed error codes                         | Measure adoption and runtime reliability                                                    | Pseudonymous account analytics; health content is forbidden                                                           | PostHog after analytics consent/GPC/DNT checks                                                                   | Product analytics operators; runtime allowlists reject recipe/profile/restriction/rule/ingredient identifiers or content, `groupId`, household/group identifiers, and subject/manager relationship fields | Proposed 90-day raw-event limit; longer trends only after documented anonymous aggregation                                                                           |
...
| Personalized data enters a public/shared recipe projection                | Persistent health inference attached to a recipe or person                | Dedicated allowlisted projection containing built-in recipe facts only; structural prohibition on profile/custom linkage                                                                                                     | Verified; retain as a release regression gate                      |
...
| A shared device reuses another account's cached result                    | Cross-account disclosure of profiles or dietary needs                     | Account-bound coordinator verifies a one-way owner marker, stops registered runtime work, and strictly purges named caches/IndexedDB on first mount, identity transition, profile deletion, and account deletion             | Dietary registration and cross-restart ownership verified by #1106 |
| Disable, sign-out, downgrade, or deletion leaves local artifacts          | Continued processing or recoverable personal data after withdrawal        | One cleanup registration boundary; bounded cancellation before strict purge; fixed failure outcomes and retry without committing the new identity; explicit downgrade behavior                                               | Product decides model-binary retention on downgrade                |
| Analytics or logs receive dietary content                                 | Secondary use, unexpected disclosure, or processor transfer               | Exact whole-event allowlist on client/server/`before_send`; unknown `dietary_*` rejection; canary tests; fixed log codes only                                                                                                | Runtime boundary verified; privacy owner reviews retention         |
...
| Account deletion retains personal correction content with a shared recipe | Under-erasure and ongoing sensitive disclosure                            | Retain only current schema-constrained allowlisted built-in facts without personal text/context; delete private, free-text, revoked, invalidated, or identifying content; remove actor linkage; projection and erasure tests | Verified; retain as a release regression gate                      |
| Superseded dietary history outlives its purpose                           | Unnecessary retention and attribution exposure                            | Daily authenticated retention job hard-deletes assessments 30 days after invalidation and corrections 30 days after revocation; erasure deletes the departing actor's stale rows immediately                                 | Implemented and covered by aggregate-count tests in #1106          |
...
- implementation evidence from #1116, #1113, and #1115, plus the #1106 reconciliation;
