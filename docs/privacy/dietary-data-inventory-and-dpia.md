...
were reconciled with the merged #1101 schema/rights, #1107 analytics, and #1109 cleanup controls.
That engineering verification does not satisfy the production-enable human gates in this document.
...
| Analytics or logs receive dietary content                                 | Secondary use, unexpected disclosure, or processor transfer               | #1107 exact whole-event allowlist on client/server/`before_send`; unknown `dietary_*` rejection; canary tests; fixed log codes only                                                                                       | Runtime boundary verified; privacy owner reviews retention          |
